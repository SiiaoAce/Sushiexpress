...
class DataRepository(QtCore.QObject):
    ...
    def _scan_and_quarantine_corrupted_parquets(self, db_name: str) -> bool:
        ...
        db_path = Path(CACHE_ROOT) / db_name / "parquet"
        ...
        def _collect_buckets(db_name: str) -> Dict[str, Set[str]]:
            ...
            while attempts < 3:
                try:
                    ...
                    err_str = str(err)
                    # Special handling for TProtocolException - scan for corrupted files
                    if "TProtocolException" in err_str or "Invalid data" in err_str:
                        self.log_message.emit(f"[FILTER] {db_name}: detected TProtocolException, scanning for corrupted parquet files...")
        ...
        if state.sales_categories:
            ...
            for category in state.sales_categories:
                if category == "DINE-IN":
                    ...
                    # For Plus: If take_away_item is NULL/empty, consider it DINE-IN (default)
                    if payment_frame.empty:
                        ...
                        sales_cat_conditions.append("""
                            (CAST(d.source_database AS VARCHAR) IN ('sushi_express_pos_live', 'sushi_plus_pos_live')
                             AND (COALESCE(CAST(d.take_away_item AS VARCHAR), '') IN ('N', '')))
                        """)
                    else:
                        sales_cat_conditions.append("""
                            (CAST(d.source_database AS VARCHAR) IN ('sushi_express_pos_live', 'sushi_plus_pos_live')
...
class FilterPanel(QtWidgets.QGroupBox):
    ...
    def __init__(self, parent: Optional[QtWidgets.QWidget] = None) -> None:
        ...
        self.bill_amount_min.textChanged.connect(self._on_filter_modified)
        ...
        self.bill_amount_max.textChanged.connect(self._on_filter_modified)
