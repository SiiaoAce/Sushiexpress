...
class OutlookDownloader:
    ...
    def download_weekly_orders(cls, destination_folder, config_file=None, account_idx=None, callback=None, progress_popup=None):
        ...
        if account_idx is None:
            ...
            main_window = None
        for supplier in supplier_dict:
            if supplier in processed_suppliers:  # 跳過已處理的供應商
                continue
            processed_suppliers.add(supplier)
            ...
            if matched_file:
                ...
                # 直接用已取得的 supplier, to_emails, cc_emails, matched_file，不再遍歷 supplier_rows
                row_frame = ctk.CTkFrame(scroll_frame)
                row_frames.append((row_frame, supplier))  # 儲存引用
                row_frame.pack(fill="x", padx=5, pady=5)  # 增加行間距
            ...
            def create_send_function(supplier, to_emails, cc_emails, attachment_file):
                def send_email():
                    ...
                    if isinstance(mail, tuple):
                        ...
                        if status_label:
                            status_label.configure(text="✗", text_color=ACCENT_RED)
                        return
                    if not mail:
                        messagebox.showerror("Error", f"无法创建邮件: {supplier}")
                        status_label = sent_status.get(supplier)
                        if status_label:
                            status_label.configure(text="✗", text_color=ACCENT_RED)
                        return
                    try:
                        mail.Send()
                        messagebox.showinfo("成功", f"郵件已直接發送給 {supplier}")
                        status_label = sent_status.get(supplier)
                        if status_label:
                            status_label.configure(text="✔", text_color=ACCENT_GREEN)
                    except Exception as e:
                        messagebox.showerror("發送失敗", f"發送郵件時發生錯誤: {str(e)}")
                        status_label = sent_status.get(supplier)
                        if status_label:
            ...
            send_btn.pack(side="right", padx=5)  # 改為右側對齊
