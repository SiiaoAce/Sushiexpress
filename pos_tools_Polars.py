...
class SalesReportFrames:
    details: pl.DataFrame
    payment_analysis: Dict[str, pl.DataFrame]
    discount_analysis: Dict[str, pl.DataFrame]
    product_analysis: Dict[str, pl.DataFrame]
    promotion_analysis: Dict[str, pl.DataFrame] = field(default_factory=dict)
    store_brand_map: pl.DataFrame = field(default_factory=pl.DataFrame)
    active_store_counts: Dict[str, int] = field(default_factory=dict)
    base_details: pl.DataFrame = field(default_factory=pl.DataFrame)
    base_payments: pl.DataFrame = field(default_factory=pl.DataFrame)
...
class DataRepository(QtCore.QObject):
    ...
    def _filter_sales_frames(
        ...
        payment_frame: pl.DataFrame,
        state: FilterState,
    ) -> Tuple[pl.DataFrame, pl.DataFrame]:
        if detail_frame.is_empty() or payment_frame.is_empty():
            return detail_frame, payment_frame

        detail = detail_frame.clone()
        payments = payment_frame.clone()

        # Normalize c_date for comparisons
        detail = detail.with_columns(
            pl.col("c_date").cast(pl.Datetime("ns"), strict=False)
        )
        ...
        # Date range filters
        if state.date_from:
            detail = detail.filter(pl.col("c_date") >= pl.lit(datetime.combine(state.date_from, datetime.min.time())))
            payments = payments.filter(pl.col("c_date") >= pl.lit(datetime.combine(state.date_from, datetime.min.time())))
        if state.date_to:
            end_of_day = datetime.combine(state.date_to, datetime.max.time())
            detail = detail.filter(pl.col("c_date") <= pl.lit(end_of_day))
            payments = payments.filter(pl.col("c_date") <= pl.lit(end_of_day))

        # Helper to build inclusion filters
        def _apply_in_filter(df: pl.DataFrame, column: str, values: List[str]) -> pl.DataFrame:
            if not values or df.is_empty() or column not in df.columns:
                return df
            return df.filter(pl.col(column).cast(pl.Utf8, strict=False).is_in(values))

        detail = _apply_in_filter(detail, "source_database", state.brands)
        detail = _apply_in_filter(detail, "store_name", state.outlets)
        detail = _apply_in_filter(detail, "item_name", state.products)
        detail = _apply_in_filter(detail, "category_code", state.categories)
        detail = _apply_in_filter(detail, "disc_name", state.discounts)

        if state.payments:
            payments = _apply_in_filter(payments, "payment_name", state.payments)

        # Weekday filter: UI uses 1=Sunday ... 7=Saturday, Polars uses 0=Monday
        if state.weekdays:
            polars_weekdays = [((weekday - 1) % 7) for weekday in state.weekdays]
            detail = detail.filter(pl.col("c_date").dt.weekday().is_in(polars_weekdays))
            payments = payments.filter(
                pl.col("c_date").dt.weekday().is_in(polars_weekdays)
            )

        if detail.is_empty() or payments.is_empty():
            return detail, payments

        # Semi-join payments against filtered detail receipt keys
        receipt_keys = (
            detail.select(
                pl.col("c_date").dt.date().alias("c_date"),
                pl.col("store_name").cast(pl.Utf8),
                pl.col("sales_no").cast(pl.Utf8),
            ).unique()
        )

        payments = payments.with_columns(pl.col("c_date").dt.date().alias("_c_date"))
        receipt_keys = receipt_keys.rename({"c_date": "_c_date"})
        payments = payments.join(receipt_keys, on=["_c_date", "store_name", "sales_no"], how="semi")
        payments = payments.drop("_c_date", strict=False)
    ...
    def fetch_sales_transactions(self, state: FilterState) -> SalesReportFrames:
        ...
        if raw_detail_frames:
            ...
            combined_detail_pd = _conn_union.execute(_sql_d).fetchdf()
            _conn_union.close()
        else:
            combined_detail_pd = pd.DataFrame()
        ...
        if raw_payment_frames:
            ...
            combined_payments_pd = _conn_union_p.execute(_sql_p).fetchdf()
            _conn_union_p.close()
        else:
            combined_payments_pd = pd.DataFrame()

        combined_detail_pl = pl.from_pandas(combined_detail_pd, include_index=False) if not combined_detail_pd.empty else pl.DataFrame()
        combined_payments_pl = pl.from_pandas(combined_payments_pd, include_index=False) if not combined_payments_pd.empty else pl.DataFrame()
        ...
        base_detail_pl, base_payments_pl = self._filter_sales_frames(combined_detail_pl, combined_payments_pl, base_state)
        base_detail = base_detail_pl.to_pandas() if not base_detail_pl.is_empty() else pd.DataFrame()
        base_payments = base_payments_pl.to_pandas() if not base_payments_pl.is_empty() else pd.DataFrame()

        filter_start = time.perf_counter()
        filtered_detail_pl, filtered_payments_pl = self._filter_sales_frames(base_detail_pl, base_payments_pl, state)
        filtered_detail = filtered_detail_pl.to_pandas() if not filtered_detail_pl.is_empty() else pd.DataFrame()
        filtered_payments = filtered_payments_pl.to_pandas() if not filtered_payments_pl.is_empty() else pd.DataFrame()
    ...
    def _build_payment_summary(
        ...
        WITH pr AS (
            SELECT
                ...
                CAST(p.tender_amount AS DOUBLE) AS tender_amount,
            ...
            FROM base b
            LEFT JOIN brand_lookup bl ON bl.store_name = b.store_name
        ), alloc_gogo AS (
            SELECT
                wb.*, 
    ...
    def _build_discount_summary(
        ...
        summary = (
            ...
            .agg(Discount_Sales=("Discount_Sales","sum"), Transaction=("Transaction","nunique"), **{ "Bill Sales": ("Bill Sales","sum") })
        )
        outlet_totals = (
            summary.groupby(["Date","WeekLabel","YearMonth","Outlet"], as_index=False)
            .agg(total_sales=("Discount_Sales","sum"), total_tx=("Transaction","sum"))
        )
        ...
        summary.drop(columns=["total_sales","total_tx"], inplace=True)
        try:
            summary = summary[(
