...
def calculate_yoy_period(date_from: date, date_to: date) -> tuple[date, date]:
    """Calculate Year-over-Year period (same ISO week number last year)
    
    Example: 
    - Current: 2025-12-02 (Tue, Week 49) to 2025-12-05 (Fri, Week 49)
    - YoY: 2024 Week 49, same weekdays → 2024-12-02 (Mon) to 2024-12-05 (Thu)
    
    Returns the same ISO week number from last year, matching the same weekdays.
    """
    from datetime import timedelta
    
    # Get ISO calendar info for current dates
    current_year, current_week, current_weekday_from = date_from.isocalendar()
    _, _, current_weekday_to = date_to.isocalendar()
    
    # Calculate last year's same week
    # Find the Monday of the target week in last year
    ...
    target_monday = week_1_monday + timedelta(weeks=current_week - 1)
    
    # Add the weekday offsets
    yoy_from = target_monday + timedelta(days=current_weekday_from - 1)
    yoy_to = target_monday + timedelta(days=current_weekday_to - 1)
    
    return yoy_from, yoy_to


def calculate_mom_period(date_from: date, date_to: date) -> tuple[date, date]:
    """Calculate Month-over-Month period (same week of month, last month)
    
    Example:
    - Current: 2025-12-02 (Tue, 1st week of Dec) to 2025-12-05 (Fri, 1st week of Dec)
    - MoM: 2025-11 1st week, same weekdays → 2025-11-03 (Mon) to 2025-11-06 (Thu)
    
    Returns the same week sequence from last month, matching the same weekdays.
    """
    from datetime import timedelta
    from calendar import monthrange
    
    # Get current month and weekday info
    current_weekday_from = date_from.weekday()  # 0=Mon, 6=Sun
    current_weekday_to = date_to.weekday()
    
    # Find which week of the month we're in
    # Week 1 starts with the first Monday of the month
    first_day_of_month = date(date_from.year, date_from.month, 1)
    first_monday = first_day_of_month
    
    # Adjust to find the first Monday
    while first_monday.weekday() != 0:  # 0 = Monday
        first_monday += timedelta(days=1)
    
    # Calculate which week of the month we're in
    days_since_first_monday = (date_from - first_monday).days
    week_of_month = days_since_first_monday // 7  # 0-indexed
    ...
    # Find the first Monday of last month
    first_day_prev_month = date(prev_year, prev_month, 1)
    first_monday_prev = first_day_prev_month
    
    while first_monday_prev.weekday() != 0:
        first_monday_prev += timedelta(days=1)
    
    # Calculate the Monday of the same week sequence in last month
    target_monday = first_monday_prev + timedelta(weeks=week_of_month)
    
    # Add the weekday offsets
    mom_from = target_monday + timedelta(days=current_weekday_from)
    mom_to = target_monday + timedelta(days=current_weekday_to)
    
    return mom_from, mom_to
...
class SalesReportFrames:
    ...
    sales_category_analysis: Dict[str, pd.DataFrame] = field(default_factory=dict)
...
class DataRepository(QtCore.QObject):
    ...
    def duckdb_connection(self, db_name: str) -> duckdb.DuckDBPyConnection:
        if db_name not in self._connections:
            ...
            try:
                ...
                self._connections[db_name].execute(f"PRAGMA threads={threads}")
                self._connections[db_name].execute("PRAGMA enable_object_cache=true")
            except Exception:
                pass
    ...
    def _brand_label(source: Optional[str], store_name: Optional[str] = None) -> str:
        """Determine brand from source_database or store_name.
        
        Priority: source_database > store_name pattern (2xx=SE, 3xx=GOGO, 4xx=PLUS)
        """
        ...
        # Fallback: determine by store_name pattern
        if store_name:
            store_str = str(store_name).strip()
            # Check for numeric prefix: 2xx=SE, 3xx=GOGO, 4xx=PLUS
            if store_str and store_str[0].isdigit():
                first_digit = store_str[0]
                if first_digit == '2':
                    return "SE"
                elif first_digit == '3':
                    return "GOGO"
            ...
            if detail_filename:
                ...
                if detail_path.exists():
                    try:
                        ...
                        self.log_message.emit(f"[CACHE] Skipping corrupted detail file: {detail_filename} ({str(e)[:100]})")
                        pass
            ...
            if payments_filename:
                ...
                if payments_path.exists():
                    try:
                        ...
                        payment_frames.append(df)
                    except Exception as e:
                        # Skip corrupted or incomplete files
                        self.log_message.emit(f"[CACHE] Skipping corrupted payment file: {payments_filename} ({str(e)[:100]})")
                        pass
    ...
    def _save_cached_raw_frames(
        ...
        if not detail_frame.empty:
            ...
            conn = self.duckdb_connection(db_name)
            conn.register("_tmp_detail_frame", detail_frame)
            conn.execute(
                "COPY (SELECT * FROM _tmp_detail_frame) TO ? (FORMAT PARQUET)",
                [detail_path.as_posix()],
            )
        ...
        if not payments_frame.empty:
            ...
            conn = self.duckdb_connection(db_name)
            conn.register("_tmp_payments_frame", payments_frame)
            conn.execute(
                "COPY (SELECT * FROM _tmp_payments_frame) TO ? (FORMAT PARQUET)",
                [payments_path.as_posix()],
            )
    ...
    def _scan_and_quarantine_corrupted_parquets(self, db_name: str) -> bool:
        """Scan all parquet files for a database and quarantine any that are corrupted.
        Returns True if any files were quarantined."""
        import pyarrow.parquet as pq
        
        cache_root = Path(".pos_cache")  # Use local cache directory
        db_path = cache_root / db_name / "parquet"
        ...
        for table_dir in db_path.iterdir():
            ...
            for parquet_file in table_dir.glob("*.parquet"):
                try:
                    # Try to open and read metadata to verify file integrity
                    pq.read_metadata(parquet_file)
                except Exception as e:
                    # File is corrupted, quarantine it
                    try:
                        counter = 0
                        while True:
                            suffix = ".corrupt" if counter == 0 else f".corrupt{counter}"
                            target = parquet_file.with_suffix(parquet_file.suffix + suffix)
                            if not target.exists():
                                parquet_file.replace(target)
                                self.log_message.emit(
                                    f"[FILTER] {db_name}: quarantined corrupted parquet '{parquet_file.name}' ({type(e).__name__})."
                                )
                                self._repair_quarantined_parquet(db_name, target)
                                quarantined_count += 1
                                break
                            counter += 1
                    except OSError as rename_err:
                        self.log_message.emit(
                            f"[FILTER] {db_name}: failed to quarantine '{parquet_file.name}' ({rename_err})."
                        )
        ...
        for table_name in TABLE_NAMES.values():
            ...
            for db in sources:
                ...
                for path in sorted(parquet_dir.glob("*.parquet")):
                    # Validate all parquet files before adding
                    try:
                        # Try to validate the file is complete and readable
                        test_conn = duckdb.connect()
                        test_conn.execute("SELECT COUNT(*) FROM read_parquet(?)", [path.as_posix()]).fetchone()
                        test_conn.close()
                        file_paths.append(path)
                    except Exception as e:
                        # Skip incomplete/corrupted files
                        self.log_message.emit(f"[FILTER] Skipping corrupted file: {path.name} ({str(e)[:50]})")
                        continue
    ...
    def load_filter_options(self, state: Optional[FilterState] = None) -> Dict[str, List[str]]:
        ...
        if state.date_from is None or state.date_to is None:
            ...
            state.date_from = _first
            state.date_to = _yesterday
        ...
        def _collect_buckets(db_name: str) -> Dict[str, Set[str]]:
            local: Dict[str, Set[str]] = {
                "outlets": set(), "months": set(), "weeks": set(), "payments": set(),
                "categories": set(), "products": set(), "discounts": set(),
            }
            try:
                conn = self.duckdb_connection(db_name)
            except duckdb.Error as exc:
                exc_str = str(exc)
                # Check if this is a corrupted parquet file error
                if any(keyword in exc_str for keyword in ["No magic bytes", "Invalid Input Error", "TProtocolException", "Invalid data"]):
                    self.log_message.emit(f"[FILTER] {db_name}: detected corrupted parquet during connection, scanning all files...")
                    quarantined_any = self._scan_and_quarantine_corrupted_parquets(db_name)
                    if quarantined_any:
                        self.log_message.emit(f"[FILTER] {db_name}: quarantined corrupted files, retrying connection...")
                        try:
                            ...
                            self.log_message.emit(f"[FILTER] {db_name}: cannot open DuckDB after quarantine ({retry_exc}).")
                            return local
                    else:
            ...
            clauses, params = _build_sales_conditions(exclude=None)
            where_sql = " AND ".join(clauses) if clauses else "1=1"
            sql = (
                "WITH s AS ("
                "  SELECT s.*, COALESCE(CAST(im.category_code AS VARCHAR), '') AS category_code "
                "  FROM v_sales_clean s "
                "  LEFT JOIN item_master im USING (item_name) "
                f"  WHERE {where_sql}"
                "), uni AS ("
                "  SELECT 'outlets' AS bucket, CAST(s.store_name AS VARCHAR) AS value FROM s WHERE s.store_name IS NOT NULL "
                "  UNION ALL SELECT 'months', strftime('%Y-%m', s.c_date) FROM s WHERE s.c_date IS NOT NULL "
                "  UNION ALL SELECT 'weeks', strftime('%Y-W%V', s.c_date) FROM s WHERE s.c_date IS NOT NULL "
                "  UNION ALL SELECT 'categories', NULLIF(s.category_code, '') FROM s WHERE s.category_code IS NOT NULL AND s.category_code != '' "
                "  UNION ALL SELECT 'discounts', CASE WHEN s.disc_name IS NULL OR TRIM(CAST(s.disc_name AS VARCHAR)) = '' THEN 'NO DISCOUNT' ELSE TRIM(CAST(s.disc_name AS VARCHAR)) END FROM s "
                "  UNION ALL SELECT 'products', CAST(s.item_name AS VARCHAR) FROM s WHERE s.item_name IS NOT NULL "
                "  UNION ALL SELECT 'payments', CAST(p.payment_name AS VARCHAR) FROM pos_sales_payment_dtls p "
                "    WHERE p.payment_name IS NOT NULL "
                "      AND EXISTS (SELECT 1 FROM s WHERE s.store_name = p.store_name AND s.sales_no = p.sales_no AND DATE(s.c_date) = DATE(p.c_date))"
                ") SELECT bucket, value FROM uni WHERE value IS NOT NULL AND value != ''"
            )
            attempts = 0
            while attempts < 3:
                try:
                    ...
                    err_str = str(err)
                    # Special handling for corrupted parquet files - scan and quarantine
                    if any(keyword in err_str for keyword in ["TProtocolException", "Invalid data", "No magic bytes", "Invalid Input Error"]):
                        self.log_message.emit(f"[FILTER] {db_name}: detected corrupted parquet, scanning all files...")
                        quarantined_any = self._scan_and_quarantine_corrupted_parquets(db_name)
                        if quarantined_any:
                            # Close and reopen connection after quarantine
                            cached_conn = self._connections.pop(db_name, None)
                            if cached_conn is not None:
                                try:
                                    cached_conn.close()
                    ...
                    # Standard quarantine logic for other errors
                    quarantined = storage_duckdb.quarantine_parquet_from_exception(err)
                    if quarantined is None:
                        self.log_message.emit(f"[FILTER] {db_name}: combined filter failed ({err}).")
                        return local
                    self.log_message.emit(
            ...
            for bucket, value in rows:
                key = str(bucket)
                if key in local and value:
                    local[key].add(str(value))
            self.log_message.emit(
                f"[FILTER] {db_name}: combined filter loaded in {time.perf_counter() - q_start:.2f}s."
            )
            return local

        worker_count = min(MAX_SYNC_WORKERS, len(targets))
        if worker_count <= 1:
            for db_name in targets:
                local = _collect_buckets(db_name)
                for k, s in local.items():
                    aggregated[k].update(s)
        else:
            with ThreadPoolExecutor(max_workers=worker_count) as executor:
                futures = {executor.submit(_collect_buckets, db): db for db in targets}
                for future in as_completed(futures):
                    db_name = futures[future]
                    try:
                        local = future.result()
                    except Exception as exc:
                        self.log_message.emit(f"[FILTER] {db_name}: combined filter failed ({exc}).")
                        continue
                    for k, s in local.items():
                        aggregated[k].update(s)
    ...
    def _load_brand_raw_frames(
        ...
        try:
            ...
            if quarantined is not None:
                self._repair_quarantined_parquet(db_name, quarantined)
                cached = self._connections.pop(db_name, None)
                if cached is not None:
                    try:
                        cached.close()
                    except Exception:
        ...
        while attempts < 2:
            try:
                ...
                quarantined = storage_duckdb.quarantine_parquet_from_exception(err)
                if quarantined is None:
                    self.log_message.emit(f"[FETCH] {db_name}: query failed ({err}).")
                    break
                self.log_message.emit(
                    f"[FETCH] {db_name}: quarantined corrupted parquet '{quarantined.name}'; repairing and retrying."
                )
                self._repair_quarantined_parquet(db_name, quarantined)
                cached_conn = self._connections.pop(db_name, None)
                if cached_conn is not None:
                ...
                try:
                    ...
                    self.log_message.emit(
                        f"[FETCH] {db_name}: unable to reopen DuckDB after quarantine ({reopen_exc})."
                    )
                    break
                attempts += 1
        try:
    ...
    def _filter_sales_frames(
        ...
        conn = duckdb.connect()
        conn.register("d", detail_frame)
        conn.register("p", payment_frame)
        clauses: List[str] = ["1=1"]
        params: List[object] = []
        if state.date_from:
            clauses.append("DATE(d.c_date) >= ?")
            params.append(state.date_from)
        if state.date_to:
            clauses.append("DATE(d.c_date) <= ?")
            params.append(state.date_to)
        if state.brands:
            placeholders = ",".join("?" for _ in state.brands)
            clauses.append(f"CAST(d.source_database AS VARCHAR) IN ({placeholders})")
            params.extend(state.brands)
        if state.outlets:
            placeholders = ",".join("?" for _ in state.outlets)
            clauses.append(f"CAST(d.store_name AS VARCHAR) IN ({placeholders})")
            params.extend(state.outlets)
        if state.products:
            placeholders = ",".join("?" for _ in state.products)
            clauses.append(f"CAST(d.item_name AS VARCHAR) IN ({placeholders})")
            params.extend(state.products)
        if state.categories:
            placeholders = ",".join("?" for _ in state.categories)
            clauses.append(f"CAST(d.category_code AS VARCHAR) IN ({placeholders})")
            params.extend(state.categories)
        if state.discounts:
            placeholders = ",".join("?" for _ in state.discounts)
            clauses.append(f"CAST(d.disc_name AS VARCHAR) IN ({placeholders})")
            params.extend(state.discounts)
        if state.weekdays:
            # Convert Python weekday (0=Monday) to database weekday (1=Sunday, 2=Monday, etc.)
            db_weekdays = [str((wd + 2) % 7 or 7) for wd in state.weekdays]
            placeholders = ",".join("?" for _ in db_weekdays)
            clauses.append(f"extract(dow from d.c_date) + 1 IN ({placeholders}) OR "
                         f"(extract(dow from d.c_date) = 6 AND 1 IN ({placeholders}))")
            params.extend(db_weekdays)
            params.extend(db_weekdays)  # Add them again for the second part of OR
        
        # Bill Amount Filter: Filter based on receipt total (using tender_amount from payments)
        if state.bill_amount_min is not None or state.bill_amount_max is not None:
            # Calculate receipt totals from payment tender amounts
            conn.execute("""
                CREATE TEMP TABLE receipt_totals AS
                SELECT 
                    CAST(store_name AS VARCHAR) AS store_name,
                    CAST(sales_no AS VARCHAR) AS sales_no,
                    DATE(c_date) AS c_date,
                    SUM(CAST(tender_amount AS DOUBLE)) AS receipt_total
                FROM p
                GROUP BY store_name, sales_no, DATE(c_date)
            """)
            
            bill_clauses = []
            bill_params = []
            if state.bill_amount_min is not None:
                bill_clauses.append("receipt_total >= ?")
                bill_params.append(state.bill_amount_min)
            if state.bill_amount_max is not None:
                bill_clauses.append("receipt_total <= ?")
                bill_params.append(state.bill_amount_max)
            
            bill_where = " AND ".join(bill_clauses)
            
            # Filter receipts by bill amount
            conn.execute(f"""
                CREATE TEMP TABLE filtered_receipts AS
                SELECT store_name, sales_no, c_date
                FROM receipt_totals
                WHERE {bill_where}
            """, bill_params)
            
            # Join back to get filtered details
            clauses.append("""
                EXISTS (
                    SELECT 1 FROM filtered_receipts fr
                    WHERE fr.store_name = CAST(d.store_name AS VARCHAR)
                    AND fr.sales_no = CAST(d.sales_no AS VARCHAR)
                    AND fr.c_date = DATE(d.c_date)
                )
            """)
        
        # Sales Category Filter: DINE-IN, TAKEAWAY, DELIVERY
        if state.sales_categories:
            # Create temp table for receipt payment methods
            # Only create if we actually have payment data
            if not payment_frame.empty:
                conn.register("p_temp", payment_frame)
                conn.execute("""
                    CREATE TEMP TABLE IF NOT EXISTS receipt_payments AS
                    SELECT DISTINCT
                        CAST(store_name AS VARCHAR) AS store_name,
                        CAST(sales_no AS VARCHAR) AS sales_no,
                        DATE(c_date) AS c_date,
            ...
            sales_cat_conditions = []
            
            for category in state.sales_categories:
                if category == "DINE-IN":
                    # DINE-IN: Express and Plus brands, not takeaway, not delivery
                    # For Plus: NULL, empty string, 'N', 'n', or missing field = DINE-IN
                    # Only 'Y' or 'y' = TAKEAWAY
                    if payment_frame.empty:
                        # No payment data - use simpler logic
                        sales_cat_conditions.append("""
                            (CAST(d.source_database AS VARCHAR) IN ('sushi_express_pos_live', 'sushi_plus_pos_live')
                             AND (CAST(d.take_away_item AS VARCHAR) IS NULL 
                                  OR CAST(d.take_away_item AS VARCHAR) = '' 
                                  OR UPPER(CAST(d.take_away_item AS VARCHAR)) = 'N'))
                        """)
                    else:
                        sales_cat_conditions.append("""
                            (CAST(d.source_database AS VARCHAR) IN ('sushi_express_pos_live', 'sushi_plus_pos_live')
                             ...
                             AND NOT EXISTS (
                                 ...
                                 WHERE rp.store_name = CAST(d.store_name AS VARCHAR)
                                 AND rp.sales_no = CAST(d.sales_no AS VARCHAR)
                                 AND rp.c_date = DATE(d.c_date)
                                 AND UPPER(rp.payment_name) IN ('FOODPANDA', 'GRABFOOD', 'DELIVEROO', 'GRAB FOOD', 'FOOD PANDA')
                             ))
                        """)
                elif category == "DELIVERY":
                    # DELIVERY: Payment method is delivery platform
                    if payment_frame.empty:
                        # No payment data - cannot filter by delivery
                        sales_cat_conditions.append("1=0")  # No results
                    else:
                        sales_cat_conditions.append("""
                            EXISTS (
                                SELECT 1 FROM receipt_payments rp
                        ...
                        """)
                elif category == "TAKEAWAY":
                    # TAKEAWAY: take_away_item = Y and not delivery
                    if payment_frame.empty:
                        # No payment data - use simpler logic
                        sales_cat_conditions.append("""
                            (COALESCE(CAST(d.take_away_item AS VARCHAR), 'N') = 'Y')
                        """)
                    else:
                        sales_cat_conditions.append("""
                            (COALESCE(CAST(d.take_away_item AS VARCHAR), 'N') = 'Y'
            ...
            if sales_cat_conditions:
                clauses.append(f"({' OR '.join(sales_cat_conditions)})")
        
        where_sql = " AND ".join(clauses)
        
        # Diagnostic logging for DINE-IN filter
        if state.sales_categories and "DINE-IN" in state.sales_categories:
            self.log_message.emit(f"[FILTER] Applying DINE-IN filter with brands: {state.brands}")
            # Check take_away_item distribution
            try:
                take_away_dist = conn.execute("""
                    SELECT 
                        source_database,
                        CAST(take_away_item AS VARCHAR) AS take_away_value,
                        COUNT(*) AS cnt
                    FROM d
                    WHERE CAST(source_database AS VARCHAR) IN ('sushi_express_pos_live', 'sushi_plus_pos_live')
                    GROUP BY source_database, CAST(take_away_item AS VARCHAR)
                    ORDER BY source_database, cnt DESC
                """).fetchdf()
                if not take_away_dist.empty:
                    self.log_message.emit(f"[FILTER] take_away_item distribution:\n{take_away_dist.to_string()}")
            except Exception as e:
                self.log_message.emit(f"[FILTER] Could not check take_away_item distribution: {e}")
        
        filtered_detail = conn.execute(
            "SELECT * FROM d WHERE " + where_sql,
            params,
        ).fetchdf()
        
        # Log filtered results
        if state.sales_categories and "DINE-IN" in state.sales_categories:
            self.log_message.emit(f"[FILTER] DINE-IN filter result: {len(filtered_detail)} rows")
        pay_clauses: List[str] = ["1=1"]
        pay_params: List[object] = []
        if state.date_from:
            pay_clauses.append("DATE(p.c_date) >= ?")
            pay_params.append(state.date_from)
        if state.date_to:
            pay_clauses.append("DATE(p.c_date) <= ?")
            pay_params.append(state.date_to)
        if state.payments:
            placeholders = ",".join("?" for _ in state.payments)
            pay_clauses.append(f"CAST(p.payment_name AS VARCHAR) IN ({placeholders})")
            pay_params.extend(state.payments)
        pay_where = " AND ".join(pay_clauses)
        # semi-join to distinct receipt keys to avoid payment row explosion
        conn.register("_fd", filtered_detail)
        filtered_payment = conn.execute(
            "WITH keys AS (SELECT DISTINCT CAST(store_name AS VARCHAR) AS store_name, CAST(sales_no AS VARCHAR) AS sales_no, DATE(c_date) AS c_date FROM _fd) "
            "SELECT DISTINCT p.* FROM p JOIN keys k "
            "ON k.store_name = p.store_name AND k.sales_no = p.sales_no AND k.c_date = DATE(p.c_date) "
            f"WHERE {pay_where}",
            pay_params,
        ).fetchdf()
        conn.close()
    ...
    def fetch_sales_transactions(self, state: FilterState) -> SalesReportFrames:
        ...
        if raw_detail_frames:
            _conn_union = duckdb.connect()
            _aliases_d: List[str] = []
            for i, df in enumerate(raw_detail_frames):
                alias = f"d{i}"
                _conn_union.register(alias, df)
                _aliases_d.append(alias)
            _sql_d = " UNION ALL ".join(f"SELECT * FROM {a}" for a in _aliases_d)
            combined_detail = _conn_union.execute(_sql_d).fetchdf()
            _conn_union.close()
        else:
            combined_detail = pd.DataFrame()

        if raw_payment_frames:
            _conn_union_p = duckdb.connect()
            _aliases_p: List[str] = []
            for i, df in enumerate(raw_payment_frames):
                alias = f"p{i}"
                _conn_union_p.register(alias, df)
                _aliases_p.append(alias)
            _sql_p = " UNION ALL ".join(f"SELECT * FROM {a}" for a in _aliases_p)
            combined_payments = _conn_union_p.execute(_sql_p).fetchdf()
            _conn_union_p.close()
        else:
        ...
        try:
            self._log_filter_diagnostics(base_detail, base_payments, state, filtered_detail, filtered_payments)
        except Exception:
            pass
    ...
    def _log_filter_diagnostics(
        ...
        conn = duckdb.connect(database=":memory:")
        conn.register("d", base_detail)
        conn.register("p", base_payments)
        clauses: List[str] = []
        params: List[object] = []
        if state.date_from:
            clauses.append("DATE(d.c_date) >= ?")
        ...
        if state.date_to:
            clauses.append("DATE(d.c_date) <= ?")
            params.append(state.date_to)
        date_where = " AND ".join(clauses) if clauses else "1=1"
        brands_where = "1=1"
        outlets_where = "1=1"
        products_where = "1=1"
        categories_where = "1=1"
        discounts_where = "1=1"
        weekdays_where = "1=1"
        br_params: List[object] = []
        if state.brands:
            placeholders = ",".join("?" for _ in state.brands)
            brands_where = f"CAST(d.source_database AS VARCHAR) IN ({placeholders})"
            br_params.extend(state.brands)
        ol_params: List[object] = []
        if state.outlets:
            placeholders = ",".join("?" for _ in state.outlets)
            outlets_where = f"CAST(d.store_name AS VARCHAR) IN ({placeholders})"
            ol_params.extend(state.outlets)
        pr_params: List[object] = []
        if state.products:
            placeholders = ",".join("?" for _ in state.products)
            products_where = f"CAST(d.item_name AS VARCHAR) IN ({placeholders})"
            pr_params.extend(state.products)
        ct_params: List[object] = []
        if state.categories:
            placeholders = ",".join("?" for _ in state.categories)
            categories_where = f"CAST(d.category_code AS VARCHAR) IN ({placeholders})"
            ct_params.extend(state.categories)
        dc_params: List[object] = []
        if state.discounts:
            placeholders = ",".join("?" for _ in state.discounts)
            discounts_where = f"CAST(d.disc_name AS VARCHAR) IN ({placeholders})"
            dc_params.extend(state.discounts)
        wd_params: List[object] = []
        if state.weekdays:
            # Convert Python weekday (0=Monday) to database weekday (1=Sunday, 2=Monday, etc.)
            db_weekdays = [str((wd + 2) % 7 or 7) for wd in state.weekdays]
            placeholders = ",".join("?" for _ in db_weekdays)
            weekdays_where = (f"(extract(dow from d.c_date) + 1 IN ({placeholders}) OR "
                           f"(extract(dow from d.c_date) = 6 AND 1 IN ({placeholders})))")
            wd_params.extend(db_weekdays)
            wd_params.extend(db_weekdays)  # Add them again for the second part of OR
        total_detail = len(base_detail.index)
        total_payment = len(base_payments.index)
        date_cnt = conn.execute(f"SELECT COUNT(*) FROM d WHERE {date_where}", params).fetchone()[0]
        brand_cnt = conn.execute(
            f"SELECT COUNT(*) FROM d WHERE {date_where} AND {brands_where}",
            [*params, *br_params],
        ).fetchone()[0]
        outlet_cnt = conn.execute(
            f"SELECT COUNT(*) FROM d WHERE {date_where} AND {brands_where} AND {outlets_where}",
            [*params, *br_params, *ol_params],
        ).fetchone()[0]
        product_cnt = conn.execute(
            f"SELECT COUNT(*) FROM d WHERE {date_where} AND {brands_where} AND {outlets_where} AND {products_where}",
            [*params, *br_params, *ol_params, *pr_params],
        ).fetchone()[0]
        category_cnt = conn.execute(
            f"SELECT COUNT(*) FROM d WHERE {date_where} AND {brands_where} AND {outlets_where} AND {products_where} AND {categories_where}",
            [*params, *br_params, *ol_params, *pr_params, *ct_params],
        ).fetchone()[0]
        discount_cnt = conn.execute(
            f"SELECT COUNT(*) FROM d WHERE {date_where} AND {brands_where} AND {outlets_where} AND {products_where} AND {categories_where} AND {discounts_where}",
            [*params, *br_params, *ol_params, *pr_params, *ct_params, *dc_params],
        ).fetchone()[0]
        weekday_cnt = conn.execute(
            f"SELECT COUNT(*) FROM d WHERE {date_where} AND {brands_where} AND {outlets_where} AND {products_where} AND {categories_where} AND {discounts_where} AND {weekdays_where}",
            [*params, *br_params, *ol_params, *pr_params, *ct_params, *dc_params, *wd_params],
        ).fetchone()[0]
        pay_date_cnt = conn.execute(
            f"SELECT COUNT(*) FROM p WHERE {date_where.replace('d.', 'p.')}",
            params,
        ).fetchone()[0]
        pay_name_cnt = pay_date_cnt
        if state.payments:
            placeholders = ",".join("?" for _ in state.payments)
            pay_name_cnt = conn.execute(
                f"SELECT COUNT(*) FROM p WHERE {date_where.replace('d.', 'p.')} AND CAST(p.payment_name AS VARCHAR) IN ({placeholders})",
                [*params, *state.payments],
            ).fetchone()[0]
        conn.close()
        self.log_message.emit(
            f"[DIAG] Details total={total_detail:,} -> date={date_cnt:,} -> brand={brand_cnt:,} -> outlet={outlet_cnt:,} -> product={product_cnt:,} -> category={category_cnt:,} -> discount={discount_cnt:,} -> weekday={weekday_cnt:,}. Final={len(filtered_detail):,}"
        )
        self.log_message.emit(
            f"[DIAG] Payments total={total_payment:,} -> date={pay_date_cnt:,} -> payment_name={pay_name_cnt:,}. Final={len(filtered_payments):,}"
        )
    ...
    def _assemble_sales_report(
        ...
        *,
        ...
        if "net_amount" in detail_frame.columns:
            net_amount_series = pd.to_numeric(detail_frame.get("net_amount"), errors="coerce").fillna(0.0)
            detail_frame["net_item"] = net_amount_series
        else:
            ...
            detail_frame["net_item"] = sub_total - pro_disc_amt - tax_deduction + svc_amt
        ...
        # Debug: Check source_database values
        if "source_database" in detail_frame.columns:
            source_dbs = detail_frame["source_database"].unique().tolist()
            print(f"[DEBUG] source_database values in detail_frame: {source_dbs}")
        ...
        # Use both source_database and store_name to determine brand
        store_brand_map["BrandGroup"] = store_brand_map.apply(
            lambda row: self._brand_label(row["source_database"], row["store_name"]), axis=1
        )
        ...
        if payment_receipt.empty:
            ...
            if base_summary is None or getattr(base_summary, "empty", True):
                simple = payment_receipt.copy()
                simple["Date"] = pd.to_datetime(simple["c_date"], errors="coerce")
                simple["YearMonth"] = simple["Date"].dt.to_period("M").astype(str)
                simple["WeekLabel"] = simple["Date"].dt.strftime("%Y-W%V")
                simple["tender"] = pd.to_numeric(simple.get("tender_amount"), errors="coerce").fillna(0.0)
                brand_lookup2 = store_brand_map.drop_duplicates(subset=["store_name"])  # store_name, BrandGroup
                simple = simple.merge(brand_lookup2, on=["store_name"], how="left")
                # attach gogo_net per receipt for GOGO allocation
                keys = ["Date", "YearMonth", "WeekLabel", "store_name", "sales_no"]
                dfd2 = detail_frame.copy()
                dfd2["Date"] = pd.to_datetime(dfd2["c_date"], errors="coerce")
                dfd2["YearMonth"] = dfd2["Date"].dt.to_period("M").astype(str)
                dfd2["WeekLabel"] = dfd2["Date"].dt.strftime("%Y-W%V")
                gogo_net2 = (
                    pd.to_numeric(dfd2.get("sub_total"), errors="coerce").fillna(0.0)
                    - pd.to_numeric(dfd2.get("pro_disc_amt"), errors="coerce").fillna(0.0)
                )
                dfd2 = dfd2.assign(gogo_net=gogo_net2)
                gogo_receipts2 = (
                    dfd2.groupby(keys, as_index=False)["gogo_net"].sum()
                )
                simple = simple.merge(gogo_receipts2, on=keys, how="left")
                simple["gogo_net"] = pd.to_numeric(simple.get("gogo_net"), errors="coerce").fillna(0.0)
                simple["is_cash"] = simple["payment_name"].astype(str).str.upper().str.contains("CASH", na=False)
                simple["is_gogo"] = simple["BrandGroup"].astype(str).eq("GOGO")
                simple["noncash_flag"] = (~simple["is_cash"]).astype(int)
                noncash_count = simple.groupby(keys)["noncash_flag"].transform("sum").fillna(0)

                # attach receipt net to each payment line to compute per-payment bill baseline
                rt_keys = ["Date", "YearMonth", "WeekLabel", "store_name", "sales_no"]
                rt = receipt_totals.copy()
                rt["Date"] = pd.to_datetime(rt["c_date"], errors="coerce")
                rt["YearMonth"] = rt["Date"].dt.to_period("M").astype(str)
                rt["WeekLabel"] = rt["Date"].dt.strftime("%Y-W%V")
                rt_attach = rt[rt_keys + ["net_total"]].rename(columns={"net_total": "bill_total"})
                simple = simple.merge(rt_attach, on=rt_keys, how="left")
                if "bill_total" not in simple.columns:
                    simple["bill_total"] = 0.0
                cap = np.where(simple["is_gogo"], simple["gogo_net"], simple["bill_total"].fillna(0.0))
                # equal split across non-cash; cash gets full cap only when no non-cash present
                simple["allocated_sales"] = np.where(
                    ~simple["is_cash"] & (noncash_count > 0),
                    cap / noncash_count,
                    np.where(simple["is_cash"] & (noncash_count == 0), cap, 0.0),
                )
                grouped = (
                    simple.groupby(["Date", "WeekLabel", "YearMonth", "store_name", "payment_name"], as_index=False)
                    .agg(payment_sales=("allocated_sales", "sum"), receipts=("sales_no", "nunique"), bill_sales=("bill_total", "sum"))
                )

                outlet_payment_totals = (
                    grouped.groupby(["Date", "WeekLabel", "YearMonth", "store_name"], as_index=False)["payment_sales"].sum()
                    .rename(columns={"payment_sales": "outlet_payment_sales"})
                )

                outlet_rollups = (
                    rt.groupby(["Date", "WeekLabel", "YearMonth", "store_name"], as_index=False)
                    .agg(outlet_bill_sales=("net_total", "sum"), outlet_receipts=("sales_no", "nunique"))
                )

                # GOGO 專用：不含稅與服務費的票據金額作為基準
                dfd = detail_frame.copy()
                dfd["Date"] = pd.to_datetime(dfd["c_date"], errors="coerce")
                dfd["YearMonth"] = dfd["Date"].dt.to_period("M").astype(str)
                dfd["WeekLabel"] = dfd["Date"].dt.strftime("%Y-W%V")
                gogo_item_net = (
                    pd.to_numeric(dfd.get("sub_total"), errors="coerce").fillna(0.0)
                    - pd.to_numeric(dfd.get("pro_disc_amt"), errors="coerce").fillna(0.0)
                )
                gogo_receipts = (
                    dfd.assign(gogo_net=gogo_item_net)
                    .groupby(["Date", "YearMonth", "WeekLabel", "store_name", "sales_no"], as_index=False)["gogo_net"].sum()
                )
                outlet_gogo_bill = (
                    gogo_receipts.groupby(["Date", "WeekLabel", "YearMonth", "store_name"], as_index=False)["gogo_net"].sum()
                    .rename(columns={"gogo_net": "outlet_bill_sales_gogo"})
                )
                brand_lookup = store_brand_map.drop_duplicates(subset=["store_name"])  # store_name, BrandGroup

                joined = grouped.merge(outlet_rollups, on=["Date", "WeekLabel", "YearMonth", "store_name"], how="left")
                joined = joined.merge(outlet_payment_totals, on=["Date", "WeekLabel", "YearMonth", "store_name"], how="left")
                joined = joined.merge(brand_lookup, on=["store_name"], how="left")
                joined = joined.merge(outlet_gogo_bill, on=["Date", "WeekLabel", "YearMonth", "store_name"], how="left")
                # keep bill_sales as per-payment sum of receipt bill totals; do not override with outlet totals
                joined["Sales Ratio %"] = np.where(
                    joined["outlet_payment_sales"].gt(0),
                    (joined["payment_sales"] / joined["outlet_payment_sales"]) * 100.0,
                    0.0,
                )
                joined["TC Ratio %"] = np.where(
                    joined["outlet_receipts"].gt(0),
                    (joined["receipts"] / joined["outlet_receipts"]) * 100.0,
                    0.0,
                )
                joined["IsTotal"] = 0

                totals_rows = outlet_rollups.merge(outlet_payment_totals, on=["Date", "WeekLabel", "YearMonth", "store_name"], how="left")
                totals_rows = totals_rows.merge(brand_lookup, on=["store_name"], how="left")
                totals_rows = totals_rows.merge(outlet_gogo_bill, on=["Date", "WeekLabel", "YearMonth", "store_name"], how="left")
                totals_rows["payment_name"] = "TOTAL"
                totals_rows.rename(
                    columns={
                        "outlet_payment_sales": "payment_sales",
                        "outlet_bill_sales": "bill_sales_sql",
                        "outlet_receipts": "receipts",
                    },
                    inplace=True,
                )
                totals_rows["bill_sales"] = np.where(
                    totals_rows["BrandGroup"].astype(str).eq("GOGO"),
                    totals_rows["outlet_bill_sales_gogo"].fillna(0.0),
                    totals_rows["bill_sales_sql"].fillna(0.0),
                )
                totals_rows["Sales Ratio %"] = 100.0
                totals_rows["TC Ratio %"] = 100.0
                totals_rows["IsTotal"] = 1

                base_summary = pd.concat([joined[[
                    "Date", "WeekLabel", "YearMonth", "store_name", "payment_name", "payment_sales", "receipts", "bill_sales", "Sales Ratio %", "TC Ratio %", "IsTotal"
                ]], totals_rows[[
                    "Date", "WeekLabel", "YearMonth", "store_name", "payment_name", "payment_sales", "receipts", "bill_sales", "Sales Ratio %", "TC Ratio %", "IsTotal"
                ]]], ignore_index=True, sort=False)
            payment_analysis = {
                mode: self._format_payment_period(base_summary, mode, store_brand_map)
                for mode in PERIOD_OPTIONS
            }
        ...
        try:
            with ThreadPoolExecutor(max_workers=4) as executor:
                fut_discount = executor.submit(self._build_discount_summary, discount_frame_source, store_brand_map, receipt_totals)
                fut_promotion = executor.submit(self._build_promotion_analysis, detail_frame.copy(), store_brand_map.copy(), state)
                fut_product = executor.submit(self._build_product_analysis, detail_frame.copy(), payments_frame.copy(), receipt_totals.copy(), store_brand_map.copy())
                fut_sales_category = executor.submit(self._build_sales_category_analysis, detail_frame.copy(), payments_frame.copy(), store_brand_map.copy())
                discount_summary = fut_discount.result()
                promotion_analysis = fut_promotion.result()
                product_analysis = fut_product.result()
                sales_category_summary = fut_sales_category.result()
                print(f"[REPORT] Sales category summary generated: shape={sales_category_summary.shape if not sales_category_summary.empty else 'empty'}, columns={list(sales_category_summary.columns) if not sales_category_summary.empty else []}")
        except Exception:
            ...
            sales_category_summary = self._build_sales_category_analysis(detail_frame, payments_frame, store_brand_map)
        ...
        sales_category_analysis = {
            mode: self._format_sales_category_period(sales_category_summary, mode, store_brand_map)
            for mode in PERIOD_OPTIONS
        }
        print(f"[REPORT] Sales category analysis formatted: {', '.join([f'{mode}={df.shape}' for mode, df in sales_category_analysis.items()])}")
    ...
    def _build_product_analysis(
        ...
        # Debug: Check available amount columns
        amount_cols = [c for c in detail_frame.columns if 'total' in c.lower() or 'item' in c.lower()]
        print(f"[PRODUCT] Available amount columns: {amount_cols}")
        ...
        # Detect staff meal receipts (payment names that include 'STAFF')
        staff_receipts = (
            payment_frame.assign(payment_name=payment_frame.get("payment_name", "").astype(str).str.upper())
            .loc[lambda df: df["payment_name"].str.contains("STAFF", na=False), ["c_date", "store_name", "sales_no"]]
            .drop_duplicates()
        )
        staff_receipts["_is_staff_receipt"] = True
        ...
        # Exclude SERVICE CHARGE, GST, and other non-product items
        item_name_upper = frame["Item Name"].str.upper()
        exclude_items = (
            item_name_upper.str.contains("SERVICE CHARGE", na=False) |
            item_name_upper.str.contains("GST", na=False) |
            item_name_upper.str.startswith("10% GST", na=False)
        )
        frame = frame[~exclude_items].copy()

        # Base numeric fields - Check all available fields and their data
        amount_cols = [c for c in frame.columns if any(x in c.lower() for x in ["amount", "total", "sub_total", "net"])]
        print(f"[PRODUCT] Available amount columns: {amount_cols}")
        
        # Check which field has most non-zero values
        if "item_sub_total" in frame.columns:
            item_sub = pd.to_numeric(frame.get("item_sub_total"), errors="coerce").fillna(0.0)
            print(f"[PRODUCT] item_sub_total: {(item_sub != 0).sum()} non-zero rows")
        if "net_item" in frame.columns:
            net_item = pd.to_numeric(frame.get("net_item"), errors="coerce").fillna(0.0)
            print(f"[PRODUCT] net_item: {(net_item != 0).sum()} non-zero rows")
        if "sub_total" in frame.columns:
            sub_total = pd.to_numeric(frame.get("sub_total"), errors="coerce").fillna(0.0)
            print(f"[PRODUCT] sub_total: {(sub_total != 0).sum()} non-zero rows")
        
        # Use the field with most data: Try all three and pick the best
        if "sub_total" in frame.columns:
            frame["Sales"] = pd.to_numeric(frame.get("sub_total"), errors="coerce").fillna(0.0)
            print(f"[PRODUCT] Using sub_total for Sales")
        elif "net_item" in frame.columns:
            frame["Sales"] = pd.to_numeric(frame.get("net_item"), errors="coerce").fillna(0.0)
            print(f"[PRODUCT] Using net_item for Sales")
        elif "item_sub_total" in frame.columns:
            frame["Sales"] = pd.to_numeric(frame.get("item_sub_total"), errors="coerce").fillna(0.0)
            print(f"[PRODUCT] Using item_sub_total for Sales")
        ...
        # Merge wastage and staff flags into detail rows so we can exclude them from sales/tx counts
        frame = frame.merge(wastage_receipts, on=["c_date", "store_name", "sales_no"], how="left")
        frame = frame.merge(staff_receipts, on=["c_date", "store_name", "sales_no"], how="left")
        wastage_flag = frame.get("_is_wastage_receipt")
        staff_flag = frame.get("_is_staff_receipt")
        ...
        if staff_flag is None:
            is_staff = pd.Series(False, index=frame.index, dtype=bool)
        else:
            is_staff = staff_flag.astype("boolean").fillna(False)

        frame["Wastage"] = np.where(is_wastage.to_numpy(dtype=bool, na_value=False), frame["Quantity"], 0.0)

        # For reporting, exclude wastage and staff-meal rows from Sales and Transaction counts.
        is_sale = (~is_wastage) & (~is_staff)
        # Zero-out sales for non-sale rows (wastage/staff meals)
        frame.loc[~is_sale, "Sales"] = 0.0
        # Null-out sales_no for non-sales so COUNT(DISTINCT sales_no) excludes them
        frame["sales_no"] = np.where(is_sale.to_numpy(dtype=bool, na_value=False), frame["sales_no"], None)
        if "_is_wastage_receipt" in frame.columns:
            frame.drop(columns=["_is_wastage_receipt"], inplace=True)
        if "_is_staff_receipt" in frame.columns:
            frame.drop(columns=["_is_staff_receipt"], inplace=True)
    ...
    def _build_promotion_analysis(
        ...
        def build_sql(mode: str) -> str:
            period_expr = {
                "Daily": "strftime('%Y-%m-%d', CAST(c_date AS TIMESTAMP))",
                "Weekly": "strftime('%Y-W%V', CAST(c_date AS TIMESTAMP))",
                "Monthly": "strftime('%Y-%m', CAST(c_date AS TIMESTAMP))",
                "Period": "'Selected Period'",
            }[mode]
            name_expr = "COALESCE(CAST(disc_name AS VARCHAR), '')" if use_discount else "COALESCE(CAST(item_name AS VARCHAR), '')"
            return f"""
            WITH base AS (
                SELECT
                    {period_expr} AS Period,
                    CAST(store_name AS VARCHAR) AS Outlet,
                    {name_expr} AS name,
                    CAST(sub_total AS DOUBLE) AS Sales,
                    CAST(qty AS DOUBLE) AS Quantity,
                    CAST(sales_no AS VARCHAR) AS Transaction
                FROM details
                WHERE {name_expr} != ''
            ), summary AS (
                SELECT Period, Outlet, name,
                    SUM(Sales) AS Sales,
                    SUM(Quantity) AS Quantity,
                    COUNT(DISTINCT Transaction) AS Transaction
                FROM base
                GROUP BY Period, Outlet, name
            ), outlet_totals AS (
                SELECT Period, Outlet,
                    SUM(Sales) AS OutletSales,
                    SUM(Transaction) AS OutletTx,
                    SUM(Quantity) AS OutletQty
                FROM summary
                GROUP BY Period, Outlet
            ), joined AS (
                SELECT s.Period, s.Outlet, s.name AS "Item / Discount Name",
                    s.Sales, s.Transaction, s.Quantity,
                    CASE WHEN ot.OutletSales > 0 THEN (s.Sales / ot.OutletSales) * 100.0 ELSE 0.0 END AS "Sales Ratio %",
                    CASE WHEN ot.OutletTx > 0 THEN (s.Transaction / ot.OutletTx) * 100.0 ELSE 0.0 END AS "Transaction Ratio %",
                    CASE WHEN ot.OutletQty > 0 THEN (s.Quantity / ot.OutletQty) * 100.0 ELSE 0.0 END AS "Quantity Ratio %"
                FROM summary s
                LEFT JOIN outlet_totals ot USING (Period, Outlet)
            ), brand_rows AS (
                SELECT j.Period, bl.BrandGroup AS Outlet, j."Item / Discount Name",
                    SUM(j.Sales) AS Sales,
                    SUM(j.Transaction) AS Transaction,
                    SUM(j.Quantity) AS Quantity,
                    100.0 AS "Sales Ratio %",
                    100.0 AS "Transaction Ratio %",
                    100.0 AS "Quantity Ratio %"
                FROM joined j
                LEFT JOIN brand_lookup bl ON bl.Outlet = j.Outlet
                WHERE bl.BrandGroup IS NOT NULL
                GROUP BY j.Period, bl.BrandGroup, j."Item / Discount Name"
            ), totals AS (
                SELECT j.Period, 'All' AS Outlet, j."Item / Discount Name",
                    SUM(j.Sales) AS Sales,
                    SUM(j.Transaction) AS Transaction,
                    SUM(j.Quantity) AS Quantity,
                    100.0 AS "Sales Ratio %",
                    ...
                    100.0 AS "Quantity Ratio %"
                FROM joined j
                GROUP BY j.Period, j."Item / Discount Name"
            )
            SELECT * FROM joined
            UNION ALL
        ...
        for mode in PERIOD_OPTIONS:
            df = conn.execute(build_sql(mode)).fetchdf()
            if df.empty:
                result[mode] = pd.DataFrame(columns=PROMOTION_ANALYSIS_COLUMNS[mode])
            else:
                df["Sales"] = pd.to_numeric(df["Sales"], errors="coerce").fillna(0.0).round(1)
                df["Transaction"] = pd.to_numeric(df["Transaction"], errors="coerce").fillna(0).astype(int)
                df["Quantity"] = pd.to_numeric(df["Quantity"], errors="coerce").fillna(0.0).round(1)
                df["Sales Ratio %"] = df["Sales Ratio %"].round(2).map(lambda x: f"{x:.2f}%")
                df["Transaction Ratio %"] = df["Transaction Ratio %"].round(2).map(lambda x: f"{x:.2f}%")
                df["Quantity Ratio %"] = df["Quantity Ratio %"].round(2).map(lambda x: f"{x:.2f}%")
                sort_fields = [f for f in PROMOTION_ANALYSIS_COLUMNS["Daily"] if f in df.columns]
                df.sort_values(sort_fields, inplace=True)
                result[mode] = df.reindex(columns=PROMOTION_ANALYSIS_COLUMNS[mode])
        conn.close()
        return result
    ...
    def _format_product_period(
        ...
        *,
        ...
        # Treat Quantity as net sales (Quantity - Wastage) for reporting.
        # Keep the separate Wastage column but present Quantity as "sold" units.
        # Clamp to 0 to avoid negative reported quantities in edge cases.
        aggregated["Quantity"] = (aggregated["Quantity"] - aggregated["Wastage"]).clip(lower=0.0)
    ...
    def _build_payment_summary(
        ...
        conn = duckdb.connect(database=":memory:")
        conn.register("payment_receipt", payment_receipt)
        conn.register("receipt_totals", receipt_totals)
        brand_lookup = store_brand_map.drop_duplicates(subset=["store_name"]).copy()
        # Ensure store_name is string type for proper JOIN
        brand_lookup["store_name"] = brand_lookup["store_name"].astype(str)
        print(f"[DEBUG] brand_lookup: {len(brand_lookup)} rows, BrandGroups: {brand_lookup['BrandGroup'].unique().tolist()}")
        conn.register("brand_lookup", brand_lookup)
        sql = """
        WITH pr AS (
            SELECT
                CAST(p.c_date AS TIMESTAMP) AS c_date,
                DATE(CAST(p.c_date AS TIMESTAMP)) AS Date,
                strftime('%Y-%m', CAST(p.c_date AS TIMESTAMP)) AS YearMonth,
                strftime('%Y-W%V', CAST(p.c_date AS TIMESTAMP)) AS WeekLabel,
                CAST(p.store_name AS VARCHAR) AS store_name,
                CAST(p.sales_no AS VARCHAR) AS sales_no,
                CAST(p.payment_name AS VARCHAR) AS payment_name,
                CAST(p.net_total AS DOUBLE) AS tender_amount,
                CAST(p.net_total AS DOUBLE) AS mx_net
            FROM payment_receipt p
        ), base AS (
            SELECT
                *,
                SUM(tender_amount) OVER (PARTITION BY Date, store_name, sales_no) AS total_tender,
                CASE WHEN UPPER(payment_name) LIKE '%CASH%' THEN 1 ELSE 0 END AS is_cash,
                SUM(CASE WHEN UPPER(payment_name) LIKE '%CASH%' THEN tender_amount ELSE 0 END) OVER (PARTITION BY Date, store_name, sales_no) AS cash_tender_total,
                SUM(CASE WHEN UPPER(payment_name) NOT LIKE '%CASH%' THEN tender_amount ELSE 0 END) OVER (PARTITION BY Date, store_name, sales_no) AS noncash_sum,
                CASE WHEN UPPER(payment_name) NOT LIKE '%CASH%' THEN tender_amount ELSE 0 END AS noncash_tender
            FROM pr
        ), with_brand AS (
            SELECT b.*, bl.BrandGroup
            FROM base b
            LEFT JOIN brand_lookup bl ON CAST(bl.store_name AS VARCHAR) = CAST(b.store_name AS VARCHAR)
        ), alloc_gogo AS (
            SELECT
                wb.*, 
                CASE WHEN wb.BrandGroup = 'GOGO' AND wb.noncash_sum > 0 AND wb.is_cash = 0
                     THEN (wb.noncash_tender / wb.noncash_sum) * wb.mx_net
                     ELSE 0 END AS noncash_alloc
            FROM with_brand wb
        ), alloc_sums AS (
            SELECT *,
                   SUM(noncash_alloc) OVER (PARTITION BY Date, store_name, sales_no) AS noncash_alloc_sum,
                   CASE WHEN cash_tender_total > 0 THEN tender_amount / cash_tender_total ELSE 0 END AS cash_share
            FROM alloc_gogo
        ), with_cash AS (
            SELECT *,
                CASE WHEN BrandGroup = 'GOGO'
                     THEN CASE WHEN is_cash = 1
                               THEN cash_share * GREATEST(mx_net - noncash_alloc_sum, 0)
                               ELSE noncash_alloc END
                     ELSE tender_amount END AS allocated_sales
            FROM alloc_sums
        ), grouped AS (
            SELECT
                Date, WeekLabel, YearMonth, store_name, sales_no, payment_name,
                SUM(allocated_sales) AS payment_sales,
                MAX(mx_net) AS bill_total
            FROM with_cash
            GROUP BY Date, WeekLabel, YearMonth, store_name, sales_no, payment_name
        ), outlet_rollups AS (
            SELECT
                DATE(CAST(r.c_date AS TIMESTAMP)) AS Date,
                strftime('%Y-%m', CAST(r.c_date AS TIMESTAMP)) AS YearMonth,
                strftime('%Y-W%V', CAST(r.c_date AS TIMESTAMP)) AS WeekLabel,
                CAST(r.store_name AS VARCHAR) AS store_name,
                SUM(CAST(r.net_total AS DOUBLE)) AS outlet_bill_sales,
                COUNT(DISTINCT CAST(r.sales_no AS VARCHAR)) AS outlet_receipts
            FROM receipt_totals r
            GROUP BY Date, WeekLabel, YearMonth, store_name
        ), outlet_payment_totals AS (
            SELECT
                Date, WeekLabel, YearMonth, store_name,
                SUM(payment_sales) AS outlet_payment_sales
            FROM grouped
            GROUP BY Date, WeekLabel, YearMonth, store_name
        ), payment_summary AS (
            SELECT
                g.Date, g.WeekLabel, g.YearMonth, g.store_name, g.payment_name,
                SUM(g.payment_sales) AS payment_sales,
                COUNT(DISTINCT g.sales_no) AS receipts,
                SUM(g.bill_total) AS bill_sales
            FROM grouped g
            GROUP BY g.Date, g.WeekLabel, g.YearMonth, g.store_name, g.payment_name
        ), joined AS (
            SELECT
                ps.Date,
                ps.WeekLabel,
                ...
                ps.receipts,
                ps.bill_sales,
                CASE WHEN opt.outlet_payment_sales > 0 THEN (ps.payment_sales / opt.outlet_payment_sales) * 100.0 ELSE 0.0 END AS "Sales Ratio %",
                CASE WHEN orp.outlet_receipts > 0 THEN (ps.receipts / orp.outlet_receipts) * 100.0 ELSE 0.0 END AS "TC Ratio %",
                0 AS IsTotal
            FROM payment_summary ps
            LEFT JOIN outlet_rollups orp USING (Date, WeekLabel, YearMonth, store_name)
            LEFT JOIN outlet_payment_totals opt USING (Date, WeekLabel, YearMonth, store_name)
        ), totals_rows AS (
            SELECT
                orp.Date,
                orp.WeekLabel,
                orp.YearMonth,
                orp.store_name,
                'TOTAL' AS payment_name,
                COALESCE(opt.outlet_payment_sales, 0) AS payment_sales,
                orp.outlet_receipts AS receipts,
                orp.outlet_bill_sales AS bill_sales,
                100.0 AS "Sales Ratio %",
                100.0 AS "TC Ratio %",
                1 AS IsTotal
            FROM outlet_rollups orp
            LEFT JOIN outlet_payment_totals opt USING (Date, WeekLabel, YearMonth, store_name)
        ), brand_totals AS (
            SELECT
                tr.Date,
                tr.WeekLabel,
                tr.YearMonth,
                bl.BrandGroup || ' TOTAL' AS store_name,
                'BRAND TOTAL' AS payment_name,
                SUM(tr.payment_sales) AS payment_sales,
                SUM(tr.receipts) AS receipts,
                SUM(tr.bill_sales) AS bill_sales,
                100.0 AS "Sales Ratio %",
                100.0 AS "TC Ratio %",
                2 AS IsTotal
            FROM totals_rows tr
            LEFT JOIN brand_lookup bl ON CAST(bl.store_name AS VARCHAR) = CAST(tr.store_name AS VARCHAR)
            WHERE bl.BrandGroup IS NOT NULL AND bl.BrandGroup != 'Other'
            GROUP BY tr.Date, tr.WeekLabel, tr.YearMonth, bl.BrandGroup
        )
        SELECT Date, WeekLabel, YearMonth, store_name, payment_name, payment_sales, receipts, bill_sales, "Sales Ratio %", "TC Ratio %", IsTotal FROM joined
        UNION ALL
        SELECT Date, WeekLabel, YearMonth, store_name, payment_name, payment_sales, receipts, bill_sales, "Sales Ratio %", "TC Ratio %", IsTotal FROM totals_rows
        UNION ALL
        SELECT Date, WeekLabel, YearMonth, store_name, payment_name, payment_sales, receipts, bill_sales, "Sales Ratio %", "TC Ratio %", IsTotal FROM brand_totals
        """
        result = self._exec_fetch_df(conn, sql)
        conn.close()
        return result

    def _format_payment_period(self, summary: pd.DataFrame, mode: str, store_brand_map: pd.DataFrame = None) -> pd.DataFrame:
        ...
        group_fields = "Period, Outlet, \"Payment Type\"" if mode != "Period" else "Outlet, \"Payment Type\""
        sql = f"""
        WITH base AS (
            SELECT
                {period_expr} AS Period,
                CAST(store_name AS VARCHAR) AS Outlet,
                CAST(payment_name AS VARCHAR) AS "Payment Type",
                CAST(payment_sales AS DOUBLE) AS payment_sales,
                CAST(receipts AS BIGINT) AS receipts,
                CAST(bill_sales AS DOUBLE) AS bill_sales,
                CAST("Sales Ratio %" AS DOUBLE) AS sales_ratio,
                CAST("TC Ratio %" AS DOUBLE) AS tc_ratio,
                CAST(IsTotal AS INTEGER) AS IsTotal
            FROM summary
        ), agg AS (
            SELECT
                {group_fields},
                SUM(payment_sales) AS PaymentSales,
                SUM(receipts) AS Receipt,
                SUM(bill_sales) AS BillSales,
                AVG(sales_ratio) AS SalesRatio,
                AVG(tc_ratio) AS TCRatio,
                MAX(IsTotal) AS IsTotal
            FROM base
            GROUP BY {group_fields}
        )
        SELECT
            {group_fields},
            ROUND(PaymentSales, 1) AS "Payment Sales",
            CAST(Receipt AS INTEGER) AS "Receipt",
            ROUND(BillSales, 1) AS "Bill Sales",
            ROUND(SalesRatio, 2) AS "Sales Ratio %",
            ROUND(TCRatio, 2) AS "TC Ratio %",
            IsTotal
        FROM agg
        """
        df = conn.execute(sql).fetchdf()
        conn.close()
        df["Sales Ratio %"] = df["Sales Ratio %"].map(lambda x: f"{x:.2f}%")
        df["TC Ratio %"] = df["TC Ratio %"].map(lambda x: f"{x:.2f}%")
        sort_fields = (["Period"] if "Period" in df.columns else []) + ["Outlet", "IsTotal", "Payment Type"]
        df.sort_values(sort_fields, inplace=True)
        df.drop(columns=["IsTotal"], inplace=True)
        if mode == "Weekly" and "Period" in df.columns:
            df["Period"] = df["Period"].astype(str).map(self._format_week_period_label)
        
        # Add Brand Total rows (aggregate TOTAL rows by brand)
        if store_brand_map is not None and not store_brand_map.empty and "Outlet" in df.columns:
            # Filter to only TOTAL rows for brand aggregation
            total_rows = df[df["Payment Type"] == "TOTAL"].copy()
            if not total_rows.empty:
                brand_lookup = store_brand_map.drop_duplicates(subset=["store_name"]).rename(columns={"store_name": "Outlet"})
                total_with_brand = total_rows.merge(brand_lookup, on="Outlet", how="left")
                total_with_brand = total_with_brand.dropna(subset=["BrandGroup"])
                total_with_brand = total_with_brand[total_with_brand["BrandGroup"] != "Other"]
                
                if not total_with_brand.empty:
                    # Group by Period (if exists) and BrandGroup
                    group_cols = (["Period"] if "Period" in total_with_brand.columns else []) + ["BrandGroup"]
                    
                    # Convert ratio strings back to numbers for aggregation
                    total_with_brand["Sales Ratio %"] = 100.0
                    total_with_brand["TC Ratio %"] = 100.0
                    
                    brand_totals = total_with_brand.groupby(group_cols, as_index=False).agg({
                        "Payment Sales": "sum",
                        "Receipt": "sum",
                        "Bill Sales": "sum",
                    })
                    brand_totals["Outlet"] = brand_totals["BrandGroup"] + " TOTAL"
                    brand_totals["Payment Type"] = "BRAND TOTAL"
                    brand_totals["Sales Ratio %"] = "100.00%"
                    brand_totals["TC Ratio %"] = "100.00%"
                    brand_totals.drop(columns=["BrandGroup"], inplace=True)
                    
                    # Ensure column order matches
                    brand_totals = brand_totals.reindex(columns=columns)
                    df = pd.concat([df, brand_totals], ignore_index=True)
    ...
    def _build_discount_summary(
        ...
        details_df = detail_frame.copy()
        if "disc_amt" not in details_df.columns:
            details_df["disc_amt"] = 0.0
        if "pro_disc_amt" not in details_df.columns:
            details_df["pro_disc_amt"] = 0.0
        conn = duckdb.connect(database=":memory:")
        conn.register("details", details_df)
        conn.register("receipt_totals", receipt_totals)
        brand_lookup = store_brand_map.drop_duplicates(subset=["store_name"]).rename(columns={"store_name": "Outlet"})
        conn.register("brand_lookup", brand_lookup)
        sql = """
        WITH base AS (
            SELECT
                DATE(CAST(d.c_date AS TIMESTAMP)) AS Date,
                strftime('%Y-%m', CAST(d.c_date AS TIMESTAMP)) AS YearMonth,
                strftime('%Y-W%V', CAST(d.c_date AS TIMESTAMP)) AS WeekLabel,
                CAST(d.store_name AS VARCHAR) AS Outlet,
                TRIM(CAST(d.disc_name AS VARCHAR)) AS "Discount Name",
                CAST(d.sales_no AS VARCHAR) AS Transaction,
                (CAST(d.disc_amt AS DOUBLE) + CAST(d.pro_disc_amt AS DOUBLE)) AS Discount_Sales,
                1.0 AS Quantity,
                CAST(d.store_name AS VARCHAR) AS store_name,
                CAST(d.sales_no AS VARCHAR) AS sales_no,
                DATE(CAST(d.c_date AS TIMESTAMP)) AS c_date
            FROM details d
            WHERE d.c_date IS NOT NULL
              AND d.disc_name IS NOT NULL
              AND TRIM(CAST(d.disc_name AS VARCHAR)) != ''
              AND (COALESCE(CAST(d.disc_amt AS DOUBLE),0) + COALESCE(CAST(d.pro_disc_amt AS DOUBLE),0)) > 0
        ), joined AS (
            SELECT b.*, CAST(r.net_total AS DOUBLE) AS ReceiptNet
            FROM base b
            LEFT JOIN (
                SELECT DATE(CAST(c_date AS TIMESTAMP)) AS c_date, CAST(store_name AS VARCHAR) AS store_name, CAST(sales_no AS VARCHAR) AS sales_no, net_total
                FROM receipt_totals
            ) r USING (c_date, store_name, sales_no)
        ), dedup AS (
            SELECT *, ROW_NUMBER() OVER (
                PARTITION BY Date, WeekLabel, YearMonth, Outlet, "Discount Name", Transaction
                ORDER BY sales_no
            ) AS rn
            FROM joined
        ), with_bill AS (
            SELECT
                Date, WeekLabel, YearMonth, Outlet, "Discount Name", Transaction,
                Discount_Sales, Quantity,
                CASE WHEN rn = 1 THEN COALESCE(ReceiptNet, 0.0) ELSE 0.0 END AS "Bill Sales"
            FROM dedup
        ), summary AS (
            SELECT
                Date, WeekLabel, YearMonth, Outlet, "Discount Name",
                SUM(Discount_Sales) AS Discount_Sales,
                COUNT(*) AS Quantity,
                COUNT(DISTINCT Transaction) AS Transaction,
                SUM("Bill Sales") AS "Bill Sales"
            FROM with_bill
            GROUP BY Date, WeekLabel, YearMonth, Outlet, "Discount Name"
        ), with_brand AS (
            SELECT s.*, bl.BrandGroup
            FROM summary s
            LEFT JOIN brand_lookup bl USING (Outlet)
        ), outlet_totals AS (
            SELECT Date, WeekLabel, YearMonth, Outlet,
                SUM(Discount_Sales) AS total_sales,
                SUM(Transaction) AS total_tx,
                SUM(Quantity) AS total_qty
            FROM with_brand
            GROUP BY Date, WeekLabel, YearMonth, Outlet
        ), ratios AS (
            SELECT wb.*, ot.total_sales, ot.total_tx, ot.total_qty,
                CASE WHEN ot.total_sales > 0 THEN (wb.Discount_Sales / ot.total_sales) * 100.0 ELSE 0.0 END AS "Discount Ratio",
                CASE WHEN ot.total_tx > 0 THEN (wb.Transaction / ot.total_tx) * 100.0 ELSE 0.0 END AS "Transaction Ratio",
                CASE WHEN ot.total_qty > 0 THEN (wb.Quantity / ot.total_qty) * 100.0 ELSE 0.0 END AS "Quantity Ratio"
            FROM with_brand wb
            LEFT JOIN outlet_totals ot USING (Date, WeekLabel, YearMonth, Outlet)
        )
        SELECT Date, WeekLabel, YearMonth, Outlet, "Discount Name",
               Discount_Sales, Quantity, Transaction, "Bill Sales",
               "Discount Ratio", "Transaction Ratio", "Quantity Ratio"
        FROM ratios
        """
        df = self._exec_fetch_df(conn, sql)
        conn.close()
        if df is not None and not getattr(df, "empty", True):
            try:
                df = df[(
                    df["Discount_Sales"].astype(float).gt(0)
                    | df["Transaction"].astype(int).gt(0)
                    | df["Bill Sales"].astype(float).gt(0)
                )]
                df = df[df["Discount Name"].astype(str).str.strip() != ""]
            except Exception:
                pass
            if not store_brand_map.empty and "Outlet" in df.columns:
                brand_lookup = (
                    store_brand_map.drop_duplicates(subset=["store_name"]).rename(columns={"store_name": "Outlet"})
                )
                brand_df = df.merge(brand_lookup, on="Outlet", how="left")
                brand_df = brand_df.dropna(subset=["BrandGroup"]) if "BrandGroup" in brand_df.columns else brand_df
                if not brand_df.empty:
                    grp_fields = ["Date", "WeekLabel", "YearMonth", "BrandGroup", "Discount Name"]
                    brand_totals = (
                        brand_df.groupby(grp_fields, as_index=False)
                        .agg(
                            Discount_Sales=("Discount_Sales", "sum"),
                            Quantity=("Quantity", "sum"),
                            Transaction=("Transaction", "sum"),
                            **{"Bill Sales": ("Bill Sales", "sum")},
                        )
                    )
                    brand_totals["Outlet"] = brand_totals["BrandGroup"].astype(str).map(lambda g: f"{g} All")
                    brand_totals["Discount Ratio"] = 100.0
                    brand_totals["Transaction Ratio"] = 100.0
                    brand_totals["Quantity Ratio"] = 100.0
                    brand_totals.drop(columns=["BrandGroup"], inplace=True, errors="ignore")
                    cols = [
                        "Date",
                        "WeekLabel",
                        "YearMonth",
                    ...
                    brand_totals = brand_totals[cols]
                    df = pd.concat([df, brand_totals], ignore_index=True, sort=False)
            return df
        dfd = detail_frame.copy()
        if dfd.empty:
            return pd.DataFrame(columns=columns)
        ...
        dfd["YearMonth"] = dfd["Date"].dt.to_period("M").astype(str)
        dfd["WeekLabel"] = dfd["Date"].dt.strftime("%Y-W%V")
        dfd["Outlet"] = dfd.get("store_name", "").astype(str)
        dfd["Discount Name"] = dfd.get("disc_name", "").astype(str).str.strip()
        disc_amt = pd.to_numeric(dfd.get("disc_amt"), errors="coerce").fillna(0.0)
        pro_disc_amt = pd.to_numeric(dfd.get("pro_disc_amt"), errors="coerce").fillna(0.0)
        dfd["Discount_Sales"] = disc_amt + pro_disc_amt
        dfd = dfd[(dfd["Discount Name"] != "") & (dfd["Discount_Sales"] > 0)]
        dfd["Quantity"] = 1.0
        dfd["Transaction"] = dfd.get("sales_no", "").astype(str)
        rt = receipt_totals.copy()
        if not rt.empty:
            rt["Date"] = pd.to_datetime(rt["c_date"], errors="coerce")
            rt["YearMonth"] = rt["Date"].dt.to_period("M").astype(str)
            rt["WeekLabel"] = rt["Date"].dt.strftime("%Y-W%V")
            rt_lookup = rt[["Date","YearMonth","WeekLabel","store_name","sales_no","net_total"]].rename(columns={"store_name":"Outlet","sales_no":"Transaction","net_total":"Bill Sales"})
            dfd = dfd.merge(rt_lookup, on=["Date","YearMonth","WeekLabel","Outlet","Transaction"], how="left")
        else:
            dfd["Bill Sales"] = 0.0
        summary = (
            dfd.groupby(["Date","WeekLabel","YearMonth","Outlet","Discount Name"], as_index=False)
            .agg(Discount_Sales=("Discount_Sales","sum"), Quantity=("Quantity","sum"), Transaction=("Transaction","nunique"), **{ "Bill Sales": ("Bill Sales","sum") })
        )
        outlet_totals = (
            summary.groupby(["Date","WeekLabel","YearMonth","Outlet"], as_index=False)
            .agg(total_sales=("Discount_Sales","sum"), total_qty=("Quantity","sum"), total_tx=("Transaction","sum"))
        )
        summary = summary.merge(outlet_totals, on=["Date","WeekLabel","YearMonth","Outlet"], how="left")
        summary["Discount Ratio"] = np.where(summary["total_sales"].gt(0),(summary["Discount_Sales"]/summary["total_sales"])*100.0,0.0)
        summary["Transaction Ratio"] = np.where(summary["total_tx"].gt(0),(summary["Transaction"]/summary["total_tx"])*100.0,0.0)
        summary["Quantity Ratio"] = np.where(summary["total_qty"].gt(0),(summary["Quantity"]/summary["total_qty"])*100.0,0.0)
        summary.drop(columns=["total_sales","total_qty","total_tx"], inplace=True)
        try:
            summary = summary[(
                summary["Discount_Sales"].astype(float).gt(0)
                | summary["Transaction"].astype(int).gt(0)
                | summary["Bill Sales"].astype(float).gt(0)
            )]
            summary = summary[summary["Discount Name"].astype(str).str.strip() != ""]
        except Exception:
            pass
        if not store_brand_map.empty:
            brand_lookup = store_brand_map.drop_duplicates(subset=["store_name"]).rename(columns={"store_name": "Outlet"})
            brand_df = summary.merge(brand_lookup, on="Outlet", how="left")
            brand_df = brand_df.dropna(subset=["BrandGroup"]) if "BrandGroup" in brand_df.columns else brand_df
            if not brand_df.empty:
                grp_fields = ["Date", "WeekLabel", "YearMonth", "BrandGroup", "Discount Name"]
                brand_totals = (
                ...
                brand_totals = brand_totals[cols]
                summary = pd.concat([summary, brand_totals], ignore_index=True, sort=False)
        return summary
    ...
    def _build_sales_category_analysis(
        self,
        detail_frame: pd.DataFrame,
        payment_frame: pd.DataFrame,
        store_brand_map: pd.DataFrame,
    ) -> pd.DataFrame:
        """Build Sales Category Analysis with Total, DINE-IN, TAKEAWAY, DELIVERY metrics"""
        columns = [
            "Date",
            "WeekLabel",
            "YearMonth",
        ...
        print(f"[SALES_CAT] detail_frame: {len(detail_frame)} rows, payment_frame: {len(payment_frame)} rows")
        print(f"[SALES_CAT] store_brand_map: {len(store_brand_map)} rows, BrandGroups: {store_brand_map['BrandGroup'].unique().tolist() if 'BrandGroup' in store_brand_map.columns else 'NO BrandGroup'}")
        print(f"[SALES_CAT] detail columns: {list(detail_frame.columns)}")
        ...
        test_sql = "SELECT COUNT(*) as cnt FROM d WHERE c_date IS NOT NULL"
        test_result = conn.execute(test_sql).fetchone()
        print(f"[SALES_CAT] detail rows with c_date: {test_result[0]}")
        ...
        SELECT COUNT(*) as cnt FROM (
            SELECT DISTINCT
                CAST(store_name AS VARCHAR) AS store_name,
                CAST(sales_no AS VARCHAR) AS sales_no,
                DATE(CAST(c_date AS TIMESTAMP)) AS c_date
            FROM p
            GROUP BY store_name, sales_no, DATE(CAST(c_date AS TIMESTAMP))
        )
        """
        payment_count = conn.execute(test_payment_sql).fetchone()
        print(f"[SALES_CAT] receipt_payments would have: {payment_count[0]} rows")
        
        sql = """
        WITH receipt_payments AS (
            SELECT DISTINCT
                CAST(store_name AS VARCHAR) AS store_name,
                CAST(sales_no AS VARCHAR) AS sales_no,
                DATE(CAST(c_date AS TIMESTAMP)) AS c_date,
                string_agg(DISTINCT CAST(payment_name AS VARCHAR), ', ') AS payment_methods
            FROM p
            GROUP BY store_name, sales_no, DATE(CAST(c_date AS TIMESTAMP))
        ), 
        detail_with_category AS (
            SELECT
                d.*,
                rp.payment_methods,
                bl.BrandGroup,
                CASE 
                    WHEN UPPER(COALESCE(rp.payment_methods, '')) LIKE '%FOODPANDA%' OR 
                         UPPER(COALESCE(rp.payment_methods, '')) LIKE '%GRABFOOD%' OR 
                         UPPER(COALESCE(rp.payment_methods, '')) LIKE '%DELIVEROO%' OR
                         UPPER(COALESCE(rp.payment_methods, '')) LIKE '%GRAB FOOD%' OR
                         UPPER(COALESCE(rp.payment_methods, '')) LIKE '%FOOD PANDA%' 
                    THEN 'DELIVERY'
                    WHEN (bl.BrandGroup = 'SE' OR bl.BrandGroup = 'PLUS') 
                         AND UPPER(CAST(COALESCE(d.take_away_item, '') AS VARCHAR)) = 'N'
                         AND NOT (UPPER(COALESCE(rp.payment_methods, '')) LIKE '%FOODPANDA%' OR 
                                  UPPER(COALESCE(rp.payment_methods, '')) LIKE '%GRABFOOD%' OR 
                                  UPPER(COALESCE(rp.payment_methods, '')) LIKE '%DELIVEROO%' OR
                                  UPPER(COALESCE(rp.payment_methods, '')) LIKE '%GRAB FOOD%' OR
                                  UPPER(COALESCE(rp.payment_methods, '')) LIKE '%FOOD PANDA%')
                    THEN 'DINE-IN'
                    WHEN UPPER(CAST(COALESCE(d.take_away_item, '') AS VARCHAR)) = 'Y'
                         AND NOT (UPPER(COALESCE(rp.payment_methods, '')) LIKE '%FOODPANDA%' OR 
                                  UPPER(COALESCE(rp.payment_methods, '')) LIKE '%GRABFOOD%' OR 
                                  UPPER(COALESCE(rp.payment_methods, '')) LIKE '%DELIVEROO%' OR
                                  UPPER(COALESCE(rp.payment_methods, '')) LIKE '%GRAB FOOD%' OR
            ...
            FROM d
            LEFT JOIN receipt_payments rp 
                ON CAST(rp.store_name AS VARCHAR) = CAST(d.store_name AS VARCHAR)
               AND CAST(rp.sales_no AS VARCHAR) = CAST(d.sales_no AS VARCHAR)
               AND rp.c_date = DATE(CAST(d.c_date AS TIMESTAMP))
            LEFT JOIN brand_lookup bl ON CAST(bl.store_name AS VARCHAR) = CAST(d.store_name AS VARCHAR)
        ),
        outlet_summary AS (
            SELECT
                DATE(CAST(c_date AS TIMESTAMP)) AS Date,
                strftime('%Y-W%V', CAST(c_date AS TIMESTAMP)) AS WeekLabel,
                strftime('%Y-%m', CAST(c_date AS TIMESTAMP)) AS YearMonth,
                CAST(store_name AS VARCHAR) AS Outlet,
                -- Total
                SUM(CAST(COALESCE(item_sub_total, net_item, sub_total) AS DOUBLE)) AS TotalSales,
                COUNT(DISTINCT CAST(sales_no AS VARCHAR)) AS TotalTC,
                SUM(CAST(qty AS DOUBLE)) AS TotalTS,
                -- DINE-IN
                SUM(CASE WHEN SalesCategory = 'DINE-IN' 
                    THEN CAST(COALESCE(item_sub_total, net_item, sub_total) AS DOUBLE) ELSE 0 END) AS DISales,
                COUNT(DISTINCT CASE WHEN SalesCategory = 'DINE-IN' 
                    THEN CAST(sales_no AS VARCHAR) END) AS DITC,
                SUM(CASE WHEN SalesCategory = 'DINE-IN' 
                    THEN CAST(qty AS DOUBLE) ELSE 0 END) AS DITS,
                -- TAKEAWAY
                SUM(CASE WHEN SalesCategory = 'TAKEAWAY' 
                    THEN CAST(COALESCE(item_sub_total, net_item, sub_total) AS DOUBLE) ELSE 0 END) AS TASales,
                COUNT(DISTINCT CASE WHEN SalesCategory = 'TAKEAWAY' 
                    THEN CAST(sales_no AS VARCHAR) END) AS TATC,
                SUM(CASE WHEN SalesCategory = 'TAKEAWAY' 
                    THEN CAST(qty AS DOUBLE) ELSE 0 END) AS TATS,
                -- DELIVERY
                SUM(CASE WHEN SalesCategory = 'DELIVERY' 
                    THEN CAST(COALESCE(item_sub_total, net_item, sub_total) AS DOUBLE) ELSE 0 END) AS DelSales,
                COUNT(DISTINCT CASE WHEN SalesCategory = 'DELIVERY' 
                    THEN CAST(sales_no AS VARCHAR) END) AS DelTC,
                SUM(CASE WHEN SalesCategory = 'DELIVERY' 
                    THEN CAST(qty AS DOUBLE) ELSE 0 END) AS DelTS
            FROM detail_with_category
            GROUP BY DATE(CAST(c_date AS TIMESTAMP)), 
                     strftime('%Y-W%V', CAST(c_date AS TIMESTAMP)), 
                     strftime('%Y-%m', CAST(c_date AS TIMESTAMP)), 
        ...
        try:
            ...
            if result is None or result.empty:
                print(f"[SALES_CAT] SQL result: empty, diagnosing CTEs...")
                ...
                SELECT COUNT(*) as cnt FROM receipt_payments
                """
                cte1_count = conn.execute(test_cte1).fetchone()
                print(f"[SALES_CAT] receipt_payments CTE: {cte1_count[0]} rows")
                ...
                LEFT JOIN brand_lookup bl ON CAST(bl.store_name AS VARCHAR) = CAST(d.store_name AS VARCHAR)
                """
                join_count = conn.execute(test_join).fetchone()
                print(f"[SALES_CAT] After JOINs: {join_count[0]} rows")
                
                # Test outlet_summary CTE
                test_outlet = """
                WITH receipt_payments AS (
                    SELECT DISTINCT
                        CAST(store_name AS VARCHAR) AS store_name,
                ...
                outlet_summary AS (
                    SELECT
                        ...
                        SUM(CAST(COALESCE(item_sub_total, net_item, sub_total) AS DOUBLE)) AS TotalSales
                    FROM detail_with_category
                    GROUP BY DATE(CAST(c_date AS TIMESTAMP)), CAST(store_name AS VARCHAR)
                )
                SELECT COUNT(*) as cnt FROM outlet_summary
                """
                outlet_count = conn.execute(test_outlet).fetchone()
                print(f"[SALES_CAT] outlet_summary CTE: {outlet_count[0]} rows")
                
                # Test full SQL directly
                try:
                    print(f"[SALES_CAT] Testing full SQL with direct execute...")
                    full_result = conn.execute(sql).fetchdf()
                    print(f"[SALES_CAT] Direct execute result: {full_result.shape}")
                    if not full_result.empty:
                        print(f"[SALES_CAT] Direct result columns: {list(full_result.columns)}")
                        print(f"[SALES_CAT] First row: {full_result.iloc[0].to_dict()}")
                        result = full_result
                except Exception as e2:
                    print(f"[SALES_CAT] Direct execute failed: {e2}")
                    import traceback
                    traceback.print_exc()
                
                if result is None or result.empty:
                    # Debug: Test simpler query
                    test_sql2 = """
                    SELECT 
                        DATE(CAST(c_date AS TIMESTAMP)) AS Date,
                        CAST(store_name AS VARCHAR) AS Outlet,
                        SUM(CAST(COALESCE(item_sub_total, net_item, sub_total, 0) AS DOUBLE)) AS TotalSales
                    FROM d
                    WHERE c_date IS NOT NULL
                    GROUP BY Date, store_name
                    LIMIT 5
                    """
                    test_result = conn.execute(test_sql2).fetchdf()
                    print(f"[SALES_CAT] Simple query result: {test_result.shape}")
                    if not test_result.empty:
                        print(f"[SALES_CAT] Sample data: {test_result.to_dict('records')[:2]}")
                    result = pd.DataFrame(columns=columns)
            else:
                print(f"[SALES_CAT] SQL result: {result.shape}")
        except Exception as e:
            import traceback
            print(f"[SALES_CAT] SQL ERROR: {e}")
            traceback.print_exc()
            result = pd.DataFrame(columns=columns)
        conn.close()
        ...
        if result is not None and not result.empty:
            for col in result.columns:
                ...
                elif 'TC' in col or 'TS' in col:
                    result[col] = result[col].astype(int)
        
        return result if result is not None else pd.DataFrame(columns=columns)

    def _format_sales_category_period(self, summary: pd.DataFrame, mode: str, store_brand_map: pd.DataFrame = None) -> pd.DataFrame:
        """Format Sales Category Analysis for specific period mode"""
        columns = [
            "Outlet",
            "Total Sales",
            "Total TC",
        ...
        grouping = ["Period", "Outlet"] if mode != "Period" else ["Outlet"]
        aggregated = (
        ...
        # Calculate TS (Ticket Size) = Sales / TC (before formatting)
        aggregated["Total TS"] = aggregated.apply(
            lambda row: row["Total Sales"] / row["Total TC"] if row["Total TC"] > 0 else 0, axis=1
        )
        aggregated["DI TS"] = aggregated.apply(
            lambda row: row["DI Sales"] / row["DI TC"] if row["DI TC"] > 0 else 0, axis=1
        )
        aggregated["TA TS"] = aggregated.apply(
            lambda row: row["TA Sales"] / row["TA TC"] if row["TA TC"] > 0 else 0, axis=1
        )
        aggregated["Del TS"] = aggregated.apply(
            lambda row: row["Del Sales"] / row["Del TC"] if row["Del TC"] > 0 else 0, axis=1
        )

        # Generate Brand Total rows if store_brand_map is provided
        if store_brand_map is not None and not store_brand_map.empty:
            # Create brand lookup - store_brand_map has columns: store_name, BrandGroup
            # Need to map to Outlet column in aggregated
            brand_lookup = store_brand_map[["store_name", "BrandGroup"]].drop_duplicates()
            brand_lookup = brand_lookup.rename(columns={"store_name": "Outlet"})
            
            # Merge brand info
            aggregated_with_brand = aggregated.merge(brand_lookup, on="Outlet", how="left")
            
            # Prepare group columns
            group_cols = ["BrandGroup"]
            if mode != "Period" and "Period" in aggregated_with_brand.columns:
                group_cols.insert(0, "Period")
            
            # Aggregate by brand (keep raw numbers for calculation)
            brand_totals = aggregated_with_brand.groupby(group_cols, as_index=False).agg({
                "Total Sales": "sum",
                "Total TC": "sum",
                "DI Sales": "sum",
                "DI TC": "sum",
            ...
            # Recalculate TS for brand totals
            brand_totals["Total TS"] = brand_totals.apply(
                lambda row: row["Total Sales"] / row["Total TC"] if row["Total TC"] > 0 else 0, axis=1
            )
            brand_totals["DI TS"] = brand_totals.apply(
                lambda row: row["DI Sales"] / row["DI TC"] if row["DI TC"] > 0 else 0, axis=1
            )
            brand_totals["TA TS"] = brand_totals.apply(
                lambda row: row["TA Sales"] / row["TA TC"] if row["TA TC"] > 0 else 0, axis=1
            )
            brand_totals["Del TS"] = brand_totals.apply(
                lambda row: row["Del Sales"] / row["Del TC"] if row["Del TC"] > 0 else 0, axis=1
            )
            
            # Set Outlet name as "BRAND TOTAL"
            brand_totals["Outlet"] = brand_totals["BrandGroup"] + " TOTAL"
            brand_totals.drop(columns=["BrandGroup"], inplace=True)
            
            # Combine aggregated data with brand totals
            aggregated = pd.concat([aggregated, brand_totals], ignore_index=True)

        # Format sales columns with $ symbol
        for col in ["Total Sales", "DI Sales", "TA Sales", "Del Sales"]:
            aggregated[col] = aggregated[col].apply(lambda x: f"$ {x:,.1f}")
        
        # Format TS (Ticket Size) columns with $ symbol
        for col in ["Total TS", "DI TS", "TA TS", "Del TS"]:
            aggregated[col] = aggregated[col].apply(lambda x: f"$ {x:,.1f}")
        
        # Format TC (Transaction Count) columns as integers
        for col in ["Total TC", "DI TC", "TA TC", "Del TC"]:
            aggregated[col] = aggregated[col].astype(int)

        if "Period" not in aggregated.columns:
            aggregated.insert(0, "Period", "Selected Period")

        sort_fields = (["Period"] if "Period" in aggregated.columns else []) + ["Outlet"]
    ...
    def _format_discount_period(self, summary: pd.DataFrame, mode: str) -> pd.DataFrame:
        ...
        aggregated["Quantity"] = aggregated["Quantity"].astype(int)
        ...
        aggregated.drop(
            columns=["Discount_Sales", "Bill_Sales", "DiscountRatio", "TransactionRatio"],
            inplace=True,
        )
...
class MultiSelectFilter(QtWidgets.QWidget):
    ...
    def selections(self) -> List[str]:
        try:
            if self._all_item is not None and self._all_item.checkState() == QtCore.Qt.CheckState.Checked:
                return []
        except Exception:
            pass
...
class SalesTransactionReportTab(QtWidgets.QWidget):
    def __init__(self, parent: Optional[QtWidgets.QWidget] = None) -> None:
        ...
        # Sales Breakdown tab
        sales_breakdown_page = QtWidgets.QWidget()
        sales_breakdown_layout = QtWidgets.QVBoxLayout(sales_breakdown_page)
        sales_breakdown_layout.setContentsMargins(0, 0, 0, 0)
        sales_breakdown_layout.setSpacing(4)

        self.sales_breakdown_label = QtWidgets.QLabel("No sales breakdown data loaded")
        self.sales_breakdown_label.setStyleSheet("color: #94a3b8;")
        sales_breakdown_layout.addWidget(self.sales_breakdown_label)

        self.sales_breakdown_model = PandasTableModel()
        self.sales_breakdown_proxy = SalesSearchProxy(self.sales_breakdown_model, self)
        self.sales_breakdown_view = QtWidgets.QTableView()
        self.sales_breakdown_view.setModel(self.sales_breakdown_proxy)
        self.sales_breakdown_view.setAlternatingRowColors(True)
        self.sales_breakdown_view.setSortingEnabled(True)
        self.sales_breakdown_view.horizontalHeader().setSectionResizeMode(QtWidgets.QHeaderView.ResizeMode.Stretch)
        self.sales_breakdown_view.setSelectionBehavior(QtWidgets.QAbstractItemView.SelectionBehavior.SelectRows)
        self.sales_breakdown_view.setSelectionMode(QtWidgets.QAbstractItemView.SelectionMode.ExtendedSelection)
        sales_breakdown_layout.addWidget(self.sales_breakdown_view, 1)

        self.tab_widget.addTab(sales_breakdown_page, "Sales Breakdown")
    ...
    def _update_discount_summary(self) -> None:
        ...
        self.discount_summary_label.setText(
            (
                "Rows: {rows:,} | Discount Sales: $ {sales:,.1f} | Quantity: {qty:,.0f} | "
                "Transactions: {txn:,.0f} | Bill Sales: $ {bill:,.1f}"
            ).format(
                rows=row_count,
    ...
    def update_sales_breakdown_analysis(self, df: pd.DataFrame) -> None:
        self.sales_breakdown_model.set_frame(df)
        self.sales_breakdown_proxy.refresh()
        if df.empty:
            self.sales_breakdown_label.setText("No sales breakdown data loaded")
            self.sales_breakdown_view.verticalHeader().setVisible(False)
            return
        
        self.sales_breakdown_view.verticalHeader().setVisible(True)
        self._update_sales_breakdown_summary(df)

    def _update_sales_breakdown_summary(self, df: pd.DataFrame) -> None:
        if df.empty:
            self.sales_breakdown_label.setText("No sales breakdown data loaded")
            return
        ...
        def _sum(column: str) -> float:
            if column not in df.columns:
                return 0.0
            col_data = df[column]
            return float(pd.to_numeric(col_data, errors="coerce").fillna(0.0).sum())
        
        total_sales = _sum("TotalSales")
        di_sales = _sum("DISales")
        ta_sales = _sum("TASales")
        del_sales = _sum("DelSales")
        
        self.sales_breakdown_label.setText(
            f"Outlets: {row_count} | Total: $ {total_sales:,.1f} | "
            f"Dine-In: $ {di_sales:,.1f} | Takeaway: $ {ta_sales:,.1f} | Delivery: $ {del_sales:,.1f}"
        )
...
class ComparisonReportStack(QtWidgets.QWidget):
    """Base class for YoY and MoM comparison reports"""
    ...
    def __init__(self, title: str, parent: Optional[QtWidgets.QWidget] = None) -> None:
        ...
        # Period info label
        self.period_label = QtWidgets.QLabel(f"{title}: No period selected")
        self.period_label.setStyleSheet("font-weight: bold; font-size: 11pt; color: #1e40af; padding: 8px;")
        ...
        # Connect selection changed for sales breakdown to update header summaries
        self._sales_breakdown_selection_model: Optional[QtCore.QItemSelectionModel] = None
        self._connect_sales_breakdown_selection()

        # Product Analytics Tab
        self.product_tab = ProductDataReportTab()
        self.tab_widget.addTab(self.product_tab, "Product Analytics")

    def set_period_info(self, current_period: str, comparison_period: str) -> None:
        """Update the period information display"""
        self.period_label.setText(
            f"{self.title} | Current: {current_period} | Comparison: {comparison_period}"
        )

    def update_payment_analysis(self, df_dict: Dict[str, pd.DataFrame], options: PaymentDisplayOptions) -> None:
        """Update payment analysis with the selected period mode"""
        mode = options.period_mode
        df = df_dict.get(mode, pd.DataFrame()).copy()
        
        # Apply show_total filter if enabled
        if options.show_total and not df.empty:
            if "Payment Type" in df.columns:
                df = df[
                    df["Payment Type"].isin(["TOTAL", "BRAND TOTAL"])
                ].copy()
    ...
    def update_discount_analysis(self, df_dict: Dict[str, pd.DataFrame]) -> None:
        """Update discount analysis - use Monthly mode by default"""
        df = df_dict.get("Monthly", pd.DataFrame())
        self.discount_model.set_frame(df)
        self.discount_proxy.refresh()
        
        if not df.empty:
            total_discount = df.get("Total Discount", pd.Series([0])).sum()
            self.discount_summary_label.setText(
                f"Total Discount Amount: RM {total_discount:,.2f}"
            )
        else:
            self.discount_summary_label.setText("No discount data loaded")

    def update_sales_breakdown(self, df_dict: Dict[str, pd.DataFrame], options: Optional[PaymentDisplayOptions] = None, 
                               day_count: int = 1, store_counts: Optional[Dict[str, int]] = None) -> None:
        """Update sales breakdown with payment options applied"""
        # Use provided options or default to Monthly mode
        mode = options.period_mode if options else "Monthly"
        df = df_dict.get(mode, pd.DataFrame()).copy()
        
        # Apply show_total filter if enabled
        if options and options.show_total and not df.empty and "Outlet" in df.columns:
            brand_total_mask = df["Outlet"].str.contains(r" TOTAL$", regex=True, na=False)
            if brand_total_mask.any():
                df = df[brand_total_mask].copy()
        
        # Apply daily average if enabled
        if options and options.daily_average and not df.empty and day_count > 1:
            numeric_columns = [col for col in ("Sales", "Discount Sales", "Net Sales", "Transaction", "Quantity") 
                             if col in df.columns]
            for col in numeric_columns:
                values = pd.to_numeric(df[col], errors="coerce").fillna(0.0)
                precision = 0 if col == "Transaction" else 1
                df[col] = (values / day_count).round(precision)
        
        # Apply outlet average if enabled
        if options and options.outlet_average and not df.empty and "Outlet" in df.columns and store_counts:
            total_mask = df["Outlet"].fillna("").str.contains(r"(?:^All$| All$)", regex=True)
            if total_mask.any():
                # Get counts for all matching rows
                counts = df.loc[total_mask, "Outlet"].map(lambda label: max(store_counts.get(label, 1), 1)).astype(float)
                numeric_columns = [col for col in ("Sales", "Discount Sales", "Net Sales", "Transaction", "Quantity") 
                                 if col in df.columns]
                for col in numeric_columns:
                    values = pd.to_numeric(df.loc[total_mask, col], errors="coerce").fillna(0.0)
                    precision = 0 if col == "Transaction" else 1
                    df.loc[total_mask, col] = (values / counts).round(precision)
        ...
        if not df.empty:
            total_sales = df.get("Sales", pd.Series([0])).sum()
            self.sales_breakdown_label.setText(
                f"Total Sales: RM {total_sales:,.2f}"
            )
            self.sales_breakdown_view.verticalHeader().setVisible(True)
            self._update_sales_breakdown_header_summaries()
        else:
            self.sales_breakdown_label.setText("No sales breakdown data loaded")
            self.sales_breakdown_view.verticalHeader().setVisible(False)
            self.sales_breakdown_model.set_header_overrides({})

    def update_product_analysis(self, df_dict: Dict[str, pd.DataFrame]) -> None:
        """Update product analysis - use Monthly mode by default"""
        df = df_dict.get("Monthly", pd.DataFrame())
        self.product_tab.update_product_analysis(df)

    def set_search_text(self, text: str) -> None:
        """Apply search filter to all views"""
        self.payment_proxy.setFilterFixedString(text)
        self.discount_proxy.setFilterFixedString(text)
        self.sales_breakdown_proxy.setFilterFixedString(text)
        self.product_tab.set_search_text(text)
    
    def _connect_sales_breakdown_selection(self) -> None:
        """Connect selection changed signal for sales breakdown view"""
        selection_model = self.sales_breakdown_view.selectionModel()
        if selection_model is self._sales_breakdown_selection_model:
            return
        if self._sales_breakdown_selection_model is not None:
            try:
                self._sales_breakdown_selection_model.selectionChanged.disconnect(self._on_sales_breakdown_selection_changed)
            except TypeError:
                pass
        self._sales_breakdown_selection_model = selection_model
        if selection_model is not None:
            selection_model.selectionChanged.connect(self._on_sales_breakdown_selection_changed)  # type: ignore[arg-type]
    
    def _on_sales_breakdown_selection_changed(self, *_: object) -> None:
        """Handle sales breakdown selection changes to update header summaries"""
        self._update_sales_breakdown_header_summaries()
    ...
    def _update_sales_breakdown_header_summaries(self) -> None:
        ...
        # Define which columns are currency, integers, or quantities
        currency_columns = {"Sales", "Discount Sales", "Net Sales"}
        integer_columns = {"Transaction"}
        quantity_columns = {"Quantity"}
        ...
        self.sales_breakdown_model.set_header_overrides(headers)
        self.sales_breakdown_view.resizeColumnsToContents()


class YoYReportStack(ComparisonReportStack):
    """Year-over-Year comparison report"""
    
    def __init__(self, parent: Optional[QtWidgets.QWidget] = None) -> None:
        super().__init__("Year-over-Year (YoY)", parent)


class MoMReportStack(ComparisonReportStack):
    """Month-over-Month comparison report"""
    
    def __init__(self, parent: Optional[QtWidgets.QWidget] = None) -> None:
        super().__init__("Month-over-Month (MoM)", parent)
...
class FilterPanel(QtWidgets.QGroupBox):
    ...
    def __init__(self, parent: Optional[QtWidgets.QWidget] = None) -> None:
        ...
        self.date_range.set_range(first_of_month.toPyDate(), yesterday.toPyDate())
        ...
        self.sales_category_filter = MultiSelectFilter("Sales Category")
        
        # Bill Amount Filter (inline with other filters)
        bill_amount_widget = QtWidgets.QGroupBox("Bill Amount")
        bill_amount_widget.setStyleSheet("""
            QGroupBox {
                font-weight: bold;
        ...
        """)
        bill_amount_layout = QtWidgets.QHBoxLayout(bill_amount_widget)
        bill_amount_layout.setContentsMargins(8, 4, 8, 8)
        bill_amount_layout.setSpacing(6)
        
        min_label = QtWidgets.QLabel("≥")
        min_label.setStyleSheet("font-size: 14px; font-weight: normal;")
        bill_amount_layout.addWidget(min_label)
        
        self.bill_amount_min = LineEdit()
        self.bill_amount_min.setPlaceholderText("Min")
        self.bill_amount_min.setFixedWidth(70)
        self.bill_amount_min.textChanged.connect(self._on_bill_amount_modified)
        bill_amount_layout.addWidget(self.bill_amount_min)
        
        max_label = QtWidgets.QLabel("≤")
        max_label.setStyleSheet("font-size: 14px; font-weight: normal;")
        bill_amount_layout.addWidget(max_label)
        
        self.bill_amount_max = LineEdit()
        self.bill_amount_max.setPlaceholderText("Max")
        self.bill_amount_max.setFixedWidth(70)
        self.bill_amount_max.textChanged.connect(self._on_bill_amount_modified)
        bill_amount_layout.addWidget(self.bill_amount_max)
        ...
        filters = [
            ...
            self.sales_category_filter,
            bill_amount_widget,  # Add bill amount filter to grid
        ]
        ...
        self._filters_debounce = QtCore.QTimer(self)
        self._filters_debounce.setSingleShot(True)
        self._filters_debounce.setInterval(150)
        self._filters_debounce.timeout.connect(self._emit_changes)
        
        # Separate debounce timer for bill amount (3 seconds)
        self._bill_amount_debounce = QtCore.QTimer(self)
        self._bill_amount_debounce.setSingleShot(True)
        self._bill_amount_debounce.setInterval(3000)  # 3 seconds
        self._bill_amount_debounce.timeout.connect(self._on_filter_modified)
    ...
    def populate(self, options: Dict[str, List[str]], *, emit: bool = False) -> None:
        ...
        self.sales_category_filter.set_options(["DINE-IN", "TAKEAWAY", "DELIVERY"], maintain=True)
    ...
    def _gather_state(self) -> FilterState:
        ...
        # Parse bill amount filters
        bill_min = None
        bill_max = None
        try:
            min_text = self.bill_amount_min.text().strip()
            if min_text:
                bill_min = float(min_text)
        except ValueError:
            pass
        
        try:
            max_text = self.bill_amount_max.text().strip()
            if max_text:
                bill_max = float(max_text)
        except ValueError:
            pass
        ...
        return FilterState(
            ...
            bill_amount_min=bill_min,
            bill_amount_max=bill_max,
            sales_categories=self.sales_category_filter.selections(),
        )

    def _on_bill_amount_modified(self, *_: object) -> None:
        """Handle bill amount input changes with 3-second delay"""
        self._filters_dirty = True
        self.apply_button.setEnabled(True)
        if self._bill_amount_debounce.isActive():
            self._bill_amount_debounce.stop()
        self._bill_amount_debounce.start()

    def _on_filter_modified(self, *_: object) -> None:
        self._filters_dirty = True
        self.apply_button.setEnabled(True)
        if self._filters_debounce.isActive():
            self._filters_debounce.stop()
        self._filters_debounce.start()
    ...
    def set_filter_controls_enabled(self, enabled: bool) -> None:
        ...
        self.bill_amount_min.setEnabled(enabled)
        self.bill_amount_max.setEnabled(enabled)
        for widget in (
            self.outlet_filter,
            self.month_filter,
            self.week_filter,
    ...
    def apply_state(self, state: FilterState) -> None:
        ...
        self.sales_category_filter.set_selections(state.sales_categories)
        
        # Apply bill amount filters
        if state.bill_amount_min is not None:
            self.bill_amount_min.setText(str(state.bill_amount_min))
        else:
            self.bill_amount_min.clear()
        
        if state.bill_amount_max is not None:
            self.bill_amount_max.setText(str(state.bill_amount_max))
        else:
            self.bill_amount_max.clear()
...
class ReportTabs(QtWidgets.QTabWidget):
    def __init__(self, parent: Optional[QtWidgets.QWidget] = None) -> None:
        ...
        # YoY and MoM comparison reports
        self.yoy_report = YoYReportStack()
        self.mom_report = MoMReportStack()
        ...
        self.addTab(self.yoy_report, "YoY Comparison")
        self.addTab(self.mom_report, "MoM Comparison")
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        self._latest_sales_breakdown_df: Dict[str, pd.DataFrame] = {mode: pd.DataFrame() for mode in PERIOD_OPTIONS}
        self._latest_store_brand_map = pd.DataFrame()
        self._latest_active_store_counts: Dict[str, int] = {}
        
        # YoY (Year-over-Year) comparison data
        self._latest_yoy_payment_df: Dict[str, pd.DataFrame] = {mode: pd.DataFrame() for mode in PERIOD_OPTIONS}
        self._latest_yoy_discount_df: Dict[str, pd.DataFrame] = {mode: pd.DataFrame() for mode in PERIOD_OPTIONS}
        self._latest_yoy_product_df: Dict[str, pd.DataFrame] = {mode: pd.DataFrame() for mode in PERIOD_OPTIONS}
        self._latest_yoy_breakdown_df: Dict[str, pd.DataFrame] = {mode: pd.DataFrame() for mode in PERIOD_OPTIONS}
        
        # MoM (Month-over-Month) comparison data
        self._latest_mom_payment_df: Dict[str, pd.DataFrame] = {mode: pd.DataFrame() for mode in PERIOD_OPTIONS}
        self._latest_mom_discount_df: Dict[str, pd.DataFrame] = {mode: pd.DataFrame() for mode in PERIOD_OPTIONS}
        self._latest_mom_product_df: Dict[str, pd.DataFrame] = {mode: pd.DataFrame() for mode in PERIOD_OPTIONS}
        self._latest_mom_breakdown_df: Dict[str, pd.DataFrame] = {mode: pd.DataFrame() for mode in PERIOD_OPTIONS}
        
        # YoY/MoM state information for daily/outlet average calculations
        self._yoy_filter_state: Optional[FilterState] = None
        self._yoy_store_brand_map = pd.DataFrame()
        self._yoy_active_store_counts: Dict[str, int] = {}
        self._mom_filter_state: Optional[FilterState] = None
        self._mom_store_brand_map = pd.DataFrame()
        self._mom_active_store_counts: Dict[str, int] = {}
        
        # Base cache for main data
        self._base_details_df = pd.DataFrame()
        self._base_payments_df = pd.DataFrame()
        self._base_filter_state = FilterState()
        
        # Base cache for YoY/MoM comparison data
        self._yoy_base_details = pd.DataFrame()
        self._yoy_base_payments = pd.DataFrame()
        self._yoy_base_state = FilterState()
        self._mom_base_details = pd.DataFrame()
        self._mom_base_payments = pd.DataFrame()
        self._mom_base_state = FilterState()
        
        # Preload configuration
        self._enable_preload = True  # Enable automatic data preloading
        ...
        self._active_cached_generation = 0
        self._cached_task_running = False
        self._queued_cached_state: Optional[FilterState] = None
        self._auto_load_pending = False  # Flag for auto-load after filters ready
        ...
        # Set flag to auto-load after filters are ready
        QtCore.QTimer.singleShot(100, self._prepare_auto_load)
    ...
    def _trigger_startup_sync(self) -> None:
        ...
        # Check and clean corrupted files, then check if yesterday's data needs syncing
        from datetime import date as date_type, timedelta
        import duckdb
        from pathlib import Path
        
        yesterday = date_type.today() - timedelta(days=1)
        brands = ["sushi_express_pos_live", "sushi_gogo_pos_live", "sushi_plus_pos_live"]
        
        # First pass: Clean ALL corrupted parquet files for all brands
        self._log("[STARTUP] Checking for corrupted parquet files...")
        total_cleaned = 0
        for brand in brands:
            base_path = Path.home() / ".pos_cache" / brand / "parquet" / "pos_sales_dtls"
            ...
            for parquet_file in parquet_files:
                try:
                    ...
                    test_conn.execute(f"SELECT COUNT(*) FROM read_parquet('{parquet_file.as_posix()}') LIMIT 1")
                    test_conn.close()
                except Exception:
                    # File is corrupted, quarantine it
                    try:
                        corrupted_path = parquet_file.with_suffix('.parquet.corrupt')
                        parquet_file.rename(corrupted_path)
                        self._log(f"[STARTUP] Quarantined corrupted file: {brand}/{parquet_file.name}")
                        total_cleaned += 1
                    except Exception as e:
                        self._log(f"[STARTUP] Failed to quarantine {parquet_file.name}: {e}")
        
        if total_cleaned > 0:
            self._log(f"[STARTUP] Cleaned {total_cleaned} corrupted file(s)")
        
        # Second pass: Check if yesterday's data is valid
        all_valid = True
        for brand in brands:
            parquet_path = Path.home() / ".pos_cache" / brand / "parquet" / "pos_sales_dtls" / f"{yesterday.isoformat()}.parquet"
            if not parquet_path.exists():
                all_valid = False
                self._log(f"[STARTUP] {brand} missing data for {yesterday.isoformat()}")
                break
        
        if all_valid:
            self._log(f"[STARTUP] Yesterday's data ({yesterday.isoformat()}) already synced, skipping startup sync.")
            self._startup_sync_requested = False
            return
        
        self._log(f"[STARTUP] Syncing yesterday's data ({yesterday.isoformat()})...")
        self._sync_remote()

    def _save_current_filter_state(self) -> None:
        """Save current filter state and cached report data to file for instant load on next startup"""
        try:
            state = self.filter_panel.current_state()
            if not state.date_from and not state.date_to and not any([
                state.outlets, state.months, state.weeks, state.payments, 
                state.categories, state.products, state.brands, state.discounts
            ]):
            ...
            state_dict = {
                ...
                "payment_options": {
                    "show_total": self._payment_options.show_total,
                    "daily_average": self._payment_options.daily_average,
                    "outlet_average": self._payment_options.outlet_average,
                    "period_mode": self._payment_options.period_mode,
                },
                "filter_options": self._cached_filter_options if hasattr(self, '_cached_filter_options') and self._cached_filter_options else {}
            }
            
            import json
            from pathlib import Path
            cache_dir = Path.home() / ".pos_cache"
            cache_dir.mkdir(exist_ok=True)
            state_file = cache_dir / "last_filter_state.json"
            
            with open(state_file, "w", encoding="utf-8") as f:
                json.dump(state_dict, f, indent=2, ensure_ascii=False)
            
            print(f"[AUTO_LOAD] Saved filter state to {state_file}")
            
            # Save cached report data for instant display
            self._save_cached_reports()
            
        except Exception as e:
            print(f"[AUTO_LOAD] Failed to save filter state: {e}")

    def _save_cached_reports(self) -> None:
        """Save current report DataFrames to pickle for instant load"""
        try:
            from pathlib import Path
            import pickle
            ...
            cache_dir = Path.home() / ".pos_cache"
            cache_dir.mkdir(exist_ok=True)
            reports_file = cache_dir / "last_reports.pkl"
            
            # Save all report DataFrames
            cached_data = {
                "payment_analysis": self._latest_payment_df,
                "discount_analysis": self._latest_discount_df,
                "product_analysis": self._latest_product_df,
                "promotion_analysis": self._latest_promotion_df,
                "sales_breakdown": self._latest_sales_breakdown_df,
                "store_brand_map": self._latest_store_brand_map,
            ...
            print(f"[AUTO_LOAD] Saved report cache to {reports_file}")
            
        except Exception as e:
            print(f"[AUTO_LOAD] Failed to save report cache: {e}")

    def _prepare_auto_load(self) -> None:
        """Prepare auto-load: check if saved state exists, load cached reports if available"""
        try:
            import json
            from pathlib import Path
            ...
            if not state_file.exists():
                print("[AUTO_LOAD] No saved filter state found")
                return
            
            # Try to load filter options cache
            try:
                with open(state_file, "r", encoding="utf-8") as f:
                    saved_state = json.load(f)
                    filter_options = saved_state.get("filter_options", {})
                    if filter_options:
                        self._cached_filter_options = filter_options
                        print(f"[AUTO_LOAD] Loaded cached filter options: {', '.join(f'{k}={len(v)}' for k, v in filter_options.items())}")
            except Exception as e:
                print(f"[AUTO_LOAD] Failed to load filter options cache: {e}")
            
            # Try to load cached reports first for instant display
            if reports_file.exists():
                try:
                    self._load_cached_reports(reports_file)
                except Exception as e:
                    print(f"[AUTO_LOAD] Failed to load report cache: {e}")
            
            # Set flag to trigger auto-load when filters are ready
            self._auto_load_pending = True
            print("[AUTO_LOAD] Saved state found, will auto-load when filters ready")
            
        except Exception as e:
            print(f"[AUTO_LOAD] Failed to check for saved state: {e}")

    def _load_cached_reports(self, reports_file) -> None:
        """Load cached report DataFrames and display immediately"""
        import pickle
        ...
        if datetime.now() - timestamp > timedelta(hours=24):
            print("[AUTO_LOAD] Report cache too old, will refresh")
            return
        
        print(f"[AUTO_LOAD] Loading cached reports from {timestamp.strftime('%Y-%m-%d %H:%M:%S')}")
        
        # Restore report DataFrames
        self._latest_payment_df = cached_data.get("payment_analysis", {})
        self._latest_discount_df = cached_data.get("discount_analysis", {})
        self._latest_product_df = cached_data.get("product_analysis", {})
        self._latest_promotion_df = cached_data.get("promotion_analysis", {})
        self._latest_sales_breakdown_df = cached_data.get("sales_breakdown", {})
        self._latest_store_brand_map = cached_data.get("store_brand_map", __import__("pandas").DataFrame())
        
        # Display cached data immediately
        self._log("[AUTO_LOAD] Displaying cached reports (data will refresh in background)")
        ...
        self._update_sales_breakdown_display()
        
        print("[AUTO_LOAD] Cached reports displayed successfully")

    def _execute_auto_load(self) -> None:
        """Execute auto-load: restore filter state and trigger data load"""
        try:
            import json
            from pathlib import Path
            ...
            print(f"[AUTO_LOAD] Restoring filter state from {state_file}")
            
            # Restore date range
            if state_dict.get("date_from") and state_dict.get("date_to"):
                date_from = date_type.fromisoformat(state_dict["date_from"])
                date_to = date_type.fromisoformat(state_dict["date_to"])
                self.filter_panel.date_range.set_range(date_from, date_to)
            
            # Restore filter selections
            filter_map = {
                "outlets": self.filter_panel.outlet_filter,
                "months": self.filter_panel.month_filter,
                "weeks": self.filter_panel.week_filter,
                "payments": self.filter_panel.payment_filter,
                "categories": self.filter_panel.category_filter,
                "products": self.filter_panel.product_filter,
                "brands": self.filter_panel.brand_filter,
                "discounts": self.filter_panel.discount_filter,
            }
            
            for key, filter_widget in filter_map.items():
                values = state_dict.get(key, [])
                if values and hasattr(filter_widget, 'set_selections'):
                    filter_widget.set_selections(values)
            
            # Restore weekday selections
            weekdays = state_dict.get("weekdays", [])
            if weekdays and hasattr(self.filter_panel, 'weekday_toggles'):
                for i, toggle in enumerate(self.filter_panel.weekday_toggles):
                    toggle.setChecked(i in weekdays)
            
            # Restore payment options
            payment_opts = state_dict.get("payment_options", {})
            if payment_opts:
                self.filter_panel.show_total_checkbox.setChecked(payment_opts.get("show_total", False))
                self.filter_panel.daily_average_checkbox.setChecked(payment_opts.get("daily_average", False))
                self.filter_panel.outlet_average_checkbox.setChecked(payment_opts.get("outlet_average", False))
                period_mode = payment_opts.get("period_mode", "Monthly")
                self.filter_panel.period_mode_combo.setCurrentText(period_mode)
                self._payment_options = PaymentDisplayOptions(
                    show_total=payment_opts.get("show_total", False),
                    daily_average=payment_opts.get("daily_average", False),
                    outlet_average=payment_opts.get("outlet_average", False),
                    period_mode=period_mode
                )
            
            print(f"[AUTO_LOAD] Filter state restored, triggering data load...")
            
            # Trigger data load immediately
            self._on_filters_changed(self.filter_panel.current_state())
            
        except Exception as e:
            print(f"[AUTO_LOAD] Failed to execute auto-load: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _on_filters_changed(self, state: FilterState) -> None:
        # Quick path: if date and brand unchanged, work like search - instant filter
        # without reloading data or filter options.
        if self._current_filter_state is not None:
            ...
            if (
                ...
                and tuple(sorted(prior.brands)) == tuple(sorted(state.brands))
            ):
                # Date and brand unchanged, use in-memory filtering only
                # NO need to reload filter options - just apply the filter instantly
                self._log("[FILTER] Quick in-memory filter (date/brand unchanged).")
                # Update current state and use cached base directly
                self._current_filter_state = state
                if not self._base_details_df.empty:
                    # Use cached base data directly - like search function
                    self._refresh_from_cached_base_async(state)
                else:
                    # No cache yet, need to fetch
                    self._refresh_sales_transactions(state)
                return
        
        # For first load or when date/brand changed, check if we need new filter options
    ...
    def _on_filters_loaded(self, state: FilterState, options: Dict[str, List[str]]) -> None:
        ...
        try:
            ...
            # Check if auto-load is pending
            if self._auto_load_pending:
                self._auto_load_pending = False
                self._log("[AUTO_LOAD] Filters ready, executing auto-load...")
                # Execute auto-load immediately in the same thread
                try:
                    self._execute_auto_load()
                except Exception as e:
                    self._log(f"[AUTO_LOAD] Failed: {e}")
                return
            
            if target_state is not None:
                self._refresh_sales_transactions(target_state)
            
            # Preload current month data in background if enabled and not yet done
            if self._enable_preload and not self._preload_completed and target_state is None:
                self._preload_completed = True
                QtCore.QTimer.singleShot(2000, self._preload_current_month_data)
        finally:
    ...
    def _handle_sync_finished(self, _result: object, generation: Optional[int] = None) -> None:
        ...
        if not self._cached_filter_options and not self._options_refresh_pending:
            QtCore.QTimer.singleShot(0, self._load_filters)
        if self._current_filter_state is not None:
            if self._active_fetch_generation == 0:
                if self._base_cache_covers(self._current_filter_state):
                    self._log("[FILTER] Reusing cached base after sync.")
                    self._refresh_from_cached_base(self._current_filter_state)
                else:
    ...
    def _preload_current_month_data(self) -> None:
        """Preload ALL available data in background for instant filtering"""
        from datetime import date as date_type, timedelta
        
        # Try to load from disk cache first
        if self._load_preload_cache():
            self._log("[CACHE] Successfully loaded from disk - skipping network preload")
            return
        
        if not self._base_details_df.empty:
            self._log("[PRELOAD] Base data already loaded, skipping preload.")
            return
        
        # Find the earliest and latest dates from cached options
        try:
            months_list = self._cached_filter_options.get("months", [])
            weeks_list = self._cached_filter_options.get("weeks", [])
            
            if not months_list and not weeks_list:
                self._log("[PRELOAD] No date info in filter options, loading current month only.")
                # Fallback to current month
                today = date_type.today()
                first_day = date_type(today.year, today.month, 1)
                if today.month == 12:
                    last_day = date_type(today.year, 12, 31)
                else:
                    next_month = date_type(today.year, today.month + 1, 1)
                    last_day = next_month - timedelta(days=1)
            else:
                # Parse all available dates and find min/max
                all_dates = []
                
                # Parse from months (YYYY-MM format)
                for month_str in months_list:
                    try:
                        year, month = map(int, month_str.split('-'))
                        all_dates.append(date_type(year, month, 1))
                    except:
                        pass
                
                # Parse from weeks (YYYY-WNN format)
                for week_str in weeks_list:
                    try:
                        import datetime
                        year_week = week_str.replace('W', '-W')
                        # Get Monday of that week
                        week_date = datetime.datetime.strptime(year_week + '-1', "%Y-W%W-%w").date()
                        all_dates.append(week_date)
                    except:
                        pass
                
                if all_dates:
                    first_day = min(all_dates)
                    last_day = date_type.today()  # Up to today
                    self._log(f"[PRELOAD] Detected date range from filter options: {first_day} to {last_day}")
                else:
                    # Fallback
                    today = date_type.today()
                    first_day = date_type(today.year, today.month, 1)
            ...
            self._log(f"[PRELOAD] Error parsing date range: {e}, using current month")
            today = date_type.today()
            first_day = date_type(today.year, today.month, 1)
            last_day = today
        
        self._log(f"[PRELOAD] 🚀 Loading ALL data ({first_day} to {last_day}) for instant filtering...")
        
        preload_state = FilterState(
            date_from=first_day,
            date_to=last_day,
            outlets=[],
            months=[],
        ...
        task = BackgroundTask(self.repository.fetch_sales_transactions, preload_state)
        task.signals.finished.connect(
            partial(self._handle_preload_data, generation=generation)
        )
        task.signals.failed.connect(
            lambda exc: self._log(f"[PRELOAD] ❌ Failed: {exc}")
        )
        self._thread_pool.start(task)

    def _handle_preload_data(self, result: SalesReportFrames, generation: int) -> None:
        """Handle preloaded data completion"""
        if generation != self._active_fetch_generation:
            self._log("[PRELOAD] Ignoring stale preload data.")
            return
        
        # Store as base cache for fast filtering
        ...
        detail_count = len(result.base_details)
        payment_count = len(result.base_payments)
        date_from = result.base_state.date_from
        date_to = result.base_state.date_to
        ...
        self._log(f"[PRELOAD] ✅ Completed: {detail_count:,} details, {payment_count:,} payments")
        self._log(f"[PRELOAD] 📅 Date range: {date_from} to {date_to} ({days_count} days)")
        self._log(f"[PRELOAD] ⚡ ALL operations will now use INSTANT in-memory filtering!")
        
        # Show notification in status bar
        self.report_tabs.sales_transaction_view.set_status(
            f"✅ Preloaded {days_count} days of data - Instant filtering enabled!"
        )
        
        # Automatically preload YoY and MoM data for the entire date range
        if date_from and date_to:
            self._log("[PRELOAD] 🔄 Now preloading YoY and MoM comparison data...")
            self._preload_yoy_mom_data(date_from, date_to)
    ...
    def _refresh_sales_transactions(self, state: Optional[FilterState] = None) -> None:
        ...
        # Always try to use cached base data first, only fetch if really needed
        if self._base_cache_covers(current_state):
            self._log("[FILTER] Reusing cached base data for filter update.")
            self._refresh_from_cached_base_async(current_state)
            return
        
        # Check if we can quickly load from local parquet without full fetch
        if self._can_load_from_local_cache(current_state):
            self._log("[FILTER] Loading from local parquet cache (date changed).")
            self._active_fetch_generation += 1
            generation = self._active_fetch_generation
            fetch_state = FilterState(
                date_from=current_state.date_from,
                date_to=current_state.date_to,
                outlets=[],
                months=[],
                weeks=[],
                payments=[],
                categories=[],
                products=[],
                brands=current_state.brands if current_state.brands else [],
                discounts=[],
                weekdays=current_state.weekdays,
            )
            task = BackgroundTask(self.repository.fetch_sales_transactions, fetch_state)
            task.signals.finished.connect(
        ...
        # Need full fetch (first load or missing data)
        self._active_fetch_generation += 1
        generation = self._active_fetch_generation
    ...
    def _can_load_from_local_cache(self, state: FilterState) -> bool:
        """Check if we can load data from local parquet cache without SQL Server fetch"""
        if not state.date_from or not state.date_to:
            return False
        
        # Check if parquet files exist for the requested date range
        from pathlib import Path
        cache_dir = Path.home() / ".pos_cache"
        
        brands = state.brands if state.brands else ["sushi_express_pos_live", "sushi_gogo_pos_live", "sushi_plus_pos_live"]
        
        # Check if we have at least some parquet files for this date range
        for brand in brands:
            parquet_dir = cache_dir / brand / "parquet" / "pos_sales_dtls"
            if parquet_dir.exists():
                # If directory exists, we can try to load from it
                return True
        
        return False

    def _base_cache_covers(self, state: FilterState) -> bool:
        """Check if base cache covers the requested state (supports date range subset)"""
        if self._base_details_df.empty:
            return False

        base_state = self._base_filter_state
        
        # Check if requested date range is within cached date range
        if not base_state.date_from or not base_state.date_to:
            return False
        
        if not state.date_from or not state.date_to:
            return False
        
        # Allow using cache if requested dates are within cached range
        if state.date_from < base_state.date_from or state.date_to > base_state.date_to:
            return False
    ...
    def _handle_sales_data(self, result: SalesReportFrames, generation: int) -> None:
        ...
        self._latest_sales_breakdown_df = {mode: df.copy() for mode, df in result.sales_category_analysis.items()}
        # Debug: Sales breakdown storage
        print(f"[STORAGE] sales_breakdown stored: {', '.join([f'{mode}={df.shape}' for mode, df in self._latest_sales_breakdown_df.items()])}")
        if result.payment_analysis:
            sample_mode = list(result.payment_analysis.keys())[0]
            sample_df = result.payment_analysis[sample_mode]
            if 'Payment Type' in sample_df.columns:
                brand_total_count = (sample_df['Payment Type'] == 'BRAND TOTAL').sum()
                print(f"[STORAGE] payment_analysis[{sample_mode}] BRAND TOTAL rows: {brand_total_count}")
        self._latest_store_brand_map = result.store_brand_map.copy(deep=True)
        self._latest_active_store_counts = dict(result.active_store_counts)
        self._base_details_df = result.base_details.copy()
        self._base_payments_df = result.base_payments.copy()
        ...
        # Trigger YoY and MoM comparison data loading
        if self._current_filter_state:
            self._refresh_yoy_data(self._current_filter_state)
            self._refresh_mom_data(self._current_filter_state)

    def _refresh_yoy_data(self, current_state: FilterState) -> None:
        """Fetch and update Year-over-Year comparison data."""
        if not current_state.date_from or not current_state.date_to:
            self._log("[YoY] No date range specified, skipping YoY comparison.")
            return
        ...
        try:
            ...
            if not self._yoy_base_details.empty and self._yoy_base_state.date_from and self._yoy_base_state.date_to:
                if (yoy_from >= self._yoy_base_state.date_from and 
                    yoy_to <= self._yoy_base_state.date_to):
                    ...
                    result = self.repository._generate_reports(
                        filtered_detail, filtered_payments, yoy_state,
                        self._latest_store_brand_map, self._latest_active_store_counts
                    )
                    
                    self._handle_yoy_data(result)
                    
                    # Update period info display
                    current_period = f"{current_state.date_from.strftime('%Y-%m-%d')} to {current_state.date_to.strftime('%Y-%m-%d')}"
                    yoy_period = f"{yoy_from.strftime('%Y-%m-%d')} to {yoy_to.strftime('%Y-%m-%d')}"
                    self.report_tabs.yoy_report.set_period_info(current_period, yoy_period)
                    return
            ...
            task.signals.finished.connect(self._handle_yoy_data)
            task.signals.failed.connect(lambda exc: self._log(f"[YoY] Fetch failed: {exc}"))
            self._thread_pool.start(task)
            ...
            self._log(f"[YoY] Error calculating period: {e}")

    def _refresh_mom_data(self, current_state: FilterState) -> None:
        """Fetch and update Month-over-Month comparison data."""
        if not current_state.date_from or not current_state.date_to:
            self._log("[MoM] No date range specified, skipping MoM comparison.")
            return
        ...
        try:
            ...
            if not self._mom_base_details.empty and self._mom_base_state.date_from and self._mom_base_state.date_to:
                if (mom_from >= self._mom_base_state.date_from and 
                    mom_to <= self._mom_base_state.date_to):
                    # Use cached data with in-memory filtering
                    ...
                    # Generate reports from filtered data
                    result = self.repository._generate_reports(
                        filtered_detail, filtered_payments, mom_state,
                        self._latest_store_brand_map, self._latest_active_store_counts
                    )
                    
                    self._handle_mom_data(result)
                    
                    # Update period info display
                    current_period = f"{current_state.date_from.strftime('%Y-%m-%d')} to {current_state.date_to.strftime('%Y-%m-%d')}"
                    mom_period = f"{mom_from.strftime('%Y-%m-%d')} to {mom_to.strftime('%Y-%m-%d')}"
                    self.report_tabs.mom_report.set_period_info(current_period, mom_period)
                    return
            ...
            task.signals.finished.connect(self._handle_mom_data)
            task.signals.failed.connect(lambda exc: self._log(f"[MoM] Fetch failed: {exc}"))
            self._thread_pool.start(task)
            ...
            self._log(f"[MoM] Error calculating period: {e}")

    def _handle_yoy_data(self, result: SalesReportFrames) -> None:
        """Handle YoY comparison data when background fetch completes."""
        try:
            self._latest_yoy_payment_df = {mode: df.copy() for mode, df in result.payment_analysis.items()}
            self._latest_yoy_discount_df = {mode: df.copy() for mode, df in result.discount_analysis.items()}
            self._latest_yoy_product_df = {mode: df.copy() for mode, df in result.product_analysis.items()}
            self._latest_yoy_breakdown_df = {mode: df.copy() for mode, df in result.sales_category_analysis.items()}
            
            # Store state and counts for daily/outlet average calculations
            self._yoy_filter_state = result.base_state
            self._yoy_store_brand_map = result.store_brand_map.copy(deep=True)
            self._yoy_active_store_counts = dict(result.active_store_counts)
            
            self._log(f"[YoY] Data loaded: {sum(len(df) for df in result.payment_analysis.values())} payment rows")
            
            # Update YoY report displays
            self._update_yoy_displays()
            
        except Exception as e:
            self._log(f"[YoY] Error handling data: {e}")

    def _handle_mom_data(self, result: SalesReportFrames) -> None:
        """Handle MoM comparison data when background fetch completes."""
        try:
            self._latest_mom_payment_df = {mode: df.copy() for mode, df in result.payment_analysis.items()}
            self._latest_mom_discount_df = {mode: df.copy() for mode, df in result.discount_analysis.items()}
            self._latest_mom_product_df = {mode: df.copy() for mode, df in result.product_analysis.items()}
            self._latest_mom_breakdown_df = {mode: df.copy() for mode, df in result.sales_category_analysis.items()}
            
            # Store state and counts for daily/outlet average calculations
            self._mom_filter_state = result.base_state
            self._mom_store_brand_map = result.store_brand_map.copy(deep=True)
            self._mom_active_store_counts = dict(result.active_store_counts)
            
            self._log(f"[MoM] Data loaded: {sum(len(df) for df in result.payment_analysis.values())} payment rows")
            
            # Update MoM report displays
            self._update_mom_displays()
            
        except Exception as e:
            self._log(f"[MoM] Error handling data: {e}")

    def _update_yoy_displays(self) -> None:
        """Update all YoY report tab displays with latest data."""
        try:
            # Update payment analysis
            self.report_tabs.yoy_report.update_payment_analysis(self._latest_yoy_payment_df, self._payment_options)
            
            # Update discount analysis
            self.report_tabs.yoy_report.update_discount_analysis(self._latest_yoy_discount_df)
            
            # Calculate day count and store counts for YoY period
            day_count = 1
            if self._yoy_filter_state and self._yoy_filter_state.date_from and self._yoy_filter_state.date_to:
                day_count = (self._yoy_filter_state.date_to - self._yoy_filter_state.date_from).days + 1
            
            # Update sales breakdown with payment options and counts
            self.report_tabs.yoy_report.update_sales_breakdown(
                self._latest_yoy_breakdown_df, 
                self._payment_options,
                day_count=day_count,
                store_counts=self._yoy_active_store_counts
            )
            
            # Update product analysis
            self.report_tabs.yoy_report.update_product_analysis(self._latest_yoy_product_df)
            
            # Apply current search text to YoY report
            if self._search_text:
                self.report_tabs.yoy_report.set_search_text(self._search_text)
            
            self._log("[YoY] All displays updated")
            
        except Exception as e:
            self._log(f"[YoY] Error updating displays: {e}")

    def _update_mom_displays(self) -> None:
        """Update all MoM report tab displays with latest data."""
        try:
            # Update payment analysis
            self.report_tabs.mom_report.update_payment_analysis(self._latest_mom_payment_df, self._payment_options)
            
            # Update discount analysis
            self.report_tabs.mom_report.update_discount_analysis(self._latest_mom_discount_df)
            
            # Calculate day count and store counts for MoM period
            day_count = 1
            if self._mom_filter_state and self._mom_filter_state.date_from and self._mom_filter_state.date_to:
                day_count = (self._mom_filter_state.date_to - self._mom_filter_state.date_from).days + 1
            
            # Update sales breakdown with payment options and counts
            self.report_tabs.mom_report.update_sales_breakdown(
                self._latest_mom_breakdown_df, 
                self._payment_options,
                day_count=day_count,
                store_counts=self._mom_active_store_counts
            )
            
            # Update product analysis
            self.report_tabs.mom_report.update_product_analysis(self._latest_mom_product_df)
            
            # Apply current search text to MoM report
            if self._search_text:
                self.report_tabs.mom_report.set_search_text(self._search_text)
            
            self._log("[MoM] All displays updated")
            
        except Exception as e:
            self._log(f"[MoM] Error updating displays: {e}")

    def _preload_yoy_mom_data(self, date_from: date, date_to: date) -> None:
        """Preload YoY and MoM comparison data for the given date range.
        This loads data in the background so it's ready immediately when needed."""
        try:
            # Calculate YoY period (last year same ISO week)
            yoy_from, yoy_to = calculate_yoy_period(date_from, date_to)
            self._log(f"[PRELOAD] Calculating YoY period: {yoy_from} to {yoy_to}")
            
            # Calculate MoM period (last month same week)
            mom_from, mom_to = calculate_mom_period(date_from, date_to)
            self._log(f"[PRELOAD] Calculating MoM period: {mom_from} to {mom_to}")
            
            # Create filter states for both periods (no filters, all brands)
            yoy_state = FilterState(
                date_from=yoy_from,
                date_to=yoy_to,
                outlets=[],
            ...
            # Fetch YoY data in background
            yoy_task = BackgroundTask(self.repository.fetch_sales_transactions, yoy_state)
            yoy_task.signals.finished.connect(self._handle_preloaded_yoy_data)
            yoy_task.signals.failed.connect(lambda exc: self._log(f"[PRELOAD] YoY fetch failed: {exc}"))
            self._thread_pool.start(yoy_task)
            
            # Fetch MoM data in background
            mom_task = BackgroundTask(self.repository.fetch_sales_transactions, mom_state)
            mom_task.signals.finished.connect(self._handle_preloaded_mom_data)
            mom_task.signals.failed.connect(lambda exc: self._log(f"[PRELOAD] MoM fetch failed: {exc}"))
            self._thread_pool.start(mom_task)
            
        except Exception as e:
            self._log(f"[PRELOAD] Error preloading YoY/MoM data: {e}")
    
    def _handle_preloaded_yoy_data(self, result: SalesReportFrames) -> None:
        """Handle preloaded YoY data - store for instant access"""
        try:
            # Store the raw base data for YoY
            self._yoy_base_details = result.base_details.copy()
            self._yoy_base_payments = result.base_payments.copy()
            self._yoy_base_state = result.base_state
            
            detail_count = len(result.base_details)
            self._log(f"[PRELOAD] ✅ YoY base data cached: {detail_count:,} details")
            
            # Check if all data is loaded, then save cache
            self._check_and_save_cache()
            
        except Exception as e:
            self._log(f"[PRELOAD] Error storing YoY base data: {e}")
    
    def _handle_preloaded_mom_data(self, result: SalesReportFrames) -> None:
        """Handle preloaded MoM data - store for instant access"""
        try:
            # Store the raw base data for MoM
            self._mom_base_details = result.base_details.copy()
            self._mom_base_payments = result.base_payments.copy()
            self._mom_base_state = result.base_state
            
            detail_count = len(result.base_details)
            self._log(f"[PRELOAD] ✅ MoM base data cached: {detail_count:,} details")
            
            # Check if all data is loaded, then save cache
            self._check_and_save_cache()
            
        except Exception as e:
            self._log(f"[PRELOAD] Error storing MoM base data: {e}")
    
    def _check_and_save_cache(self) -> None:
        """Check if all preload data is ready and save to disk cache"""
        # Check if all three datasets are loaded
        if (not self._base_details_df.empty and 
            not self._yoy_base_details.empty and 
            not self._mom_base_details.empty):
            self._log("[CACHE] 💾 All preload data ready, saving to disk...")
            self._save_preload_cache()
    
    def _save_preload_cache(self) -> None:
        """Save all preloaded data to disk for instant loading next time"""
        try:
            import pickle
            cache_dir = Path(".pos_cache_preload")
            cache_dir.mkdir(exist_ok=True)
            
            cache_data = {
                'main_details': self._base_details_df,
                'main_payments': self._base_payments_df,
                'main_state': self._base_filter_state,
                'yoy_details': self._yoy_base_details,
                'yoy_payments': self._yoy_base_payments,
                'yoy_state': self._yoy_base_state,
                'mom_details': self._mom_base_details,
                'mom_payments': self._mom_base_payments,
                'mom_state': self._mom_base_state,
                'store_brand_map': self._latest_store_brand_map,
                'active_store_counts': self._latest_active_store_counts,
                'timestamp': datetime.now(),
            }
            
            cache_file = cache_dir / "full_preload_cache.pkl"
            with open(cache_file, 'wb') as f:
                pickle.dump(cache_data, f, protocol=pickle.HIGHEST_PROTOCOL)
            
            file_size_mb = cache_file.stat().st_size / (1024 * 1024)
            self._log(f"[CACHE] ✅ Saved complete cache ({file_size_mb:.1f} MB) - Next startup will be instant!")
            
        except Exception as e:
            self._log(f"[CACHE] ⚠️ Failed to save cache: {e}")
    
    def _load_preload_cache(self) -> bool:
        """Load preloaded data from disk cache if available and recent"""
        try:
            import pickle
            cache_file = Path(".pos_cache_preload") / "full_preload_cache.pkl"
            ...
            if cache_age.days >= 1:
                self._log("[CACHE] Cache is older than 1 day, will refresh")
                return False
            
            self._log("[CACHE] 📂 Loading preloaded cache from disk...")
            
            with open(cache_file, 'rb') as f:
                cache_data = pickle.load(f)
            
            # Restore all cached data
            self._base_details_df = cache_data['main_details']
            self._base_payments_df = cache_data['main_payments']
            self._base_filter_state = cache_data['main_state']
            self._yoy_base_details = cache_data['yoy_details']
            self._yoy_base_payments = cache_data['yoy_payments']
            self._yoy_base_state = cache_data['yoy_state']
            self._mom_base_details = cache_data['mom_details']
            self._mom_base_payments = cache_data['mom_payments']
            self._mom_base_state = cache_data['mom_state']
            self._latest_store_brand_map = cache_data['store_brand_map']
            self._latest_active_store_counts = cache_data['active_store_counts']
            
            detail_count = len(self._base_details_df)
            yoy_count = len(self._yoy_base_details)
            mom_count = len(self._mom_base_details)
            cache_timestamp = cache_data['timestamp']
            
            self._log(f"[CACHE] ✅ Loaded cached data from {cache_timestamp.strftime('%Y-%m-%d %H:%M')}")
            self._log(f"[CACHE] Main: {detail_count:,} details, YoY: {yoy_count:,}, MoM: {mom_count:,}")
            self._log(f"[CACHE] ⚡ All data ready - instant filtering enabled!")
            
            self.report_tabs.sales_transaction_view.set_status(
                "✅ Loaded cached data - Instant filtering enabled!"
            )
            
            return True
            
        except Exception as e:
            self._log(f"[CACHE] ⚠️ Failed to load cache: {e}")
            return False
    ...
    def _refresh_from_cached_base(self, state: FilterState) -> None:
        ...
        if filtered_detail.empty or filtered_payments.empty:
            ...
            empty_payment = {
                "Monthly": pd.DataFrame(columns=["Period", "Outlet", "Payment Type", "Payment Sales", "Receipt", "Bill Sales", "Sales Ratio %", "TC Ratio %"]),
                "Weekly": pd.DataFrame(columns=["Period", "Outlet", "Payment Type", "Payment Sales", "Receipt", "Bill Sales", "Sales Ratio %", "TC Ratio %"]),
                "Period": pd.DataFrame(columns=["Outlet", "Payment Type", "Payment Sales", "Receipt", "Bill Sales", "Sales Ratio %", "TC Ratio %"]),
            }
            empty_discount = {mode: pd.DataFrame(columns=DISCOUNT_ANALYSIS_COLUMNS[mode]) for mode in PERIOD_OPTIONS}
            empty_product = {mode: pd.DataFrame(columns=PRODUCT_ANALYSIS_COLUMNS[mode]) for mode in PERIOD_OPTIONS}
    ...
    def _refresh_from_cached_base_async(self, state: FilterState) -> None:
        if self._base_details_df.empty:
            self._refresh_sales_transactions(state)
            return
        filtered_detail, filtered_payments = self.repository._filter_sales_frames(
        ...
        if filtered_detail.empty or filtered_payments.empty:
            self._refresh_from_cached_base(state)
            return
        if self._cached_task_running:
            self._queued_cached_state = state
            return
        self._cached_task_running = True
        self._active_cached_generation += 1
        generation = self._active_cached_generation
        task = BackgroundTask(
            self.repository._assemble_sales_report,
            filtered_detail,
            filtered_payments,
            base_details=self._base_details_df.copy(),
            base_payments=self._base_payments_df.copy(),
        ...
        task.signals.finished.connect(partial(self._handle_cached_finished, generation=generation, state=state))
        task.signals.failed.connect(self._handle_sync_failed)
        self._thread_pool.start(task)

    def _handle_cached_finished(self, result: SalesReportFrames, generation: int, state: FilterState) -> None:
        if generation != self._active_cached_generation:
            return
        formatted_details = self._format_sales_dataframe(result.details)
        self.report_tabs.sales_transaction_report.details_view.update_data(formatted_details)
        if formatted_details.empty:
        ...
        # Trigger YoY and MoM comparison data loading
        if state:
            self._refresh_yoy_data(state)
            self._refresh_mom_data(state)
        
        self._cached_task_running = False
        if self._queued_cached_state is not None:
            queued = self._queued_cached_state
            self._queued_cached_state = None
            QtCore.QTimer.singleShot(0, partial(self._refresh_from_cached_base_async, queued))
    ...
    def _on_payment_options_changed(
        ...
        # Also update YoY and MoM displays with new payment options
        if self._latest_yoy_payment_df:
            self._update_yoy_displays()
        if self._latest_mom_payment_df:
            self._update_mom_displays()
        
        # Update YoY and MoM displays with new payment options
        self._update_yoy_displays()
        self._update_mom_displays()

    def _selected_day_count(self) -> int:
        state = self._current_filter_state
        if state and state.date_from and state.date_to:
            start = state.date_from
            end = state.date_to
            total = (end - start).days + 1
            if state.weekdays:
                selected = set(int(x) for x in state.weekdays)
                count = 0
        ...
        if not self._latest_details_df.empty:
            dates = pd.to_datetime(self._latest_details_df["Date"], errors="coerce").dt.normalize()
            if state and state.weekdays:
                wk = dates.dt.weekday
                mask = wk.isin([int(x) for x in state.weekdays])
                return int(dates[mask].nunique()) if mask.any() else 1
    ...
    def _update_payment_analysis_display(self) -> None:
        ...
        display = base.copy()
        # Debug: Check for BRAND TOTAL rows
        if "Payment Type" in display.columns:
            brand_total_count = (display["Payment Type"] == "BRAND TOTAL").sum()
            total_count = (display["Payment Type"] == "TOTAL").sum()
            print(f"[PAYMENT] mode={current_mode}, total_rows={len(display)}, TOTAL_rows={total_count}, BRAND_TOTAL_rows={brand_total_count}")
        
        if self._payment_options.show_total:
            # When show_total is True, only show TOTAL and BRAND TOTAL rows
            if "Payment Type" in display.columns:
                display = display[
                    display["Payment Type"].isin(["TOTAL", "BRAND TOTAL"])
                ].copy()
    ...
    def _update_discount_analysis_display(self) -> None:
        ...
        if (
            ...
            day_count = max(self._selected_day_count(), 1)
            if day_count > 1:
                for column in ("Discount Sales", "Transaction"):
        ...
        if self._payment_options.outlet_average and not frame.empty and "Outlet" in frame.columns:
            ...
            if total_mask.any():
                ...
                for column, precision in (("Discount Sales", 1), ("Transaction", 1)):
                    if column in frame.columns:
        ...
        # Update Sales Breakdown Analysis
        self._update_sales_breakdown_display()

    def _update_sales_breakdown_display(self) -> None:
        current_mode = self._payment_options.period_mode
        base = self._latest_sales_breakdown_df.get(current_mode, pd.DataFrame())
        
        self._log(f"[BREAKDOWN] Updating display for mode={current_mode}, has_data={not base.empty}, shape={base.shape if not base.empty else 'empty'}")
        
        if base.empty:
            self._log("[BREAKDOWN] No data available")
            self.report_tabs.sales_transaction_report.update_sales_breakdown_analysis(base)
            return

        display = base.copy()
        
        # When show_total is True, only show BRAND TOTAL rows
        if self._payment_options.show_total and "Outlet" in display.columns:
            brand_total_mask = display["Outlet"].str.contains(r" TOTAL$", regex=True, na=False)
            if brand_total_mask.any():
                display = display[brand_total_mask].copy()
        
        # Apply daily average if enabled
        if self._payment_options.daily_average and not display.empty:
            day_count = max(self._selected_day_count(), 1)
            if day_count > 1:
                # Apply to numeric columns: Sales, Transaction, Quantity, etc.
                numeric_columns = [col for col in ("Sales", "Discount Sales", "Net Sales", "Transaction", "Quantity") 
                                 if col in display.columns]
                for col in numeric_columns:
                    values = pd.to_numeric(display[col], errors="coerce").fillna(0.0)
        ...
        display.reset_index(drop=True, inplace=True)
        self._log(f"[BREAKDOWN] Displaying {len(display)} rows")
        self.report_tabs.sales_transaction_report.update_sales_breakdown_analysis(display)
